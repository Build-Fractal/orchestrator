...
| D015 | M045 re-entry substrate: process-fresh, not in-session ScheduleWakeup | P01 viability spike (VERDICT PARTIAL) showed in-session ScheduleWakeup re-entry does not reset context and gives no per-rotation relief (weight analog compounds 4->11->18; relief depends on non-rotation-aware harness compaction). Folding M045 into v2b's process-fresh approach: on rotation, hand off to a fresh claude -p re-entry that starts a genuinely fresh context and resumes from disk. P02-P04 mechanism work (deterministic branch, capability detection, safety envelope, observability) carries over ~unchanged; only the re-entry primitive swaps from ScheduleWakeup to claude -p. | architecture | M045/P01 | In-session re-entry is structurally weaker than today's fresh-session re-invoke on the exact axis rotation targets; process-fresh re-entry is the only substrate that restores a genuine per-rotation context reset (spec CON-5 route). Operator-confirmed 2026-07-01. | No |
| D016 | #Q-7: wall-clock ceiling value/mechanism for the unattended envelope — fixed internal constant or operator-supplied? | Operator-supplied per-run flag --max-wall-clock-s (whole-run ceiling, seconds) with NO default on the unattended path; the driver refuses to start under --unattended if it is unset (FR-13 fail-closed enumeration) and enforces it live via the mid-segment watchdog against a run-start deadline plus a pre-spawn check; the attended path is unaffected (no wall-clock enforcement, FR-17 parity) | scope | M046/P04 | FR-13's text presumes the ceiling can be unset (refuse-to-start if unset) — a fixed internal constant can never be unset, which would make FR-13's wall-clock enumeration vacuous; CON-4 always-on is preserved because refuse-to-start makes presence mandatory on every unattended run, and the operator knows the overnight window better than any one-size constant | Yes |
| D017 | How is the production default-DENY PreToolUse scope hook activated so it constrains ONLY the unattended child, never the operator's interactive session? | Env-gated default-OFF: the hook is always wired into settings.json (matcher Write|Edit|Bash|mcp__.*) but its first action is a no-op-and-exit-0 unless ORCHESTRATOR_UNATTENDED is set in the environment; the M046 driver already exports ORCHESTRATOR_UNATTENDED=1 into the spawned child (self-continue-drive.sh run_child), so enforcement is scoped to that child while the operator's own session (no env var) is unaffected. | arch | M046/P05 | Safety-critical correctness point: a permanently-installed deny hook must not constrain interactive work; env-gating keyed to the driver-exported unattended flag is the narrowest activation that still survives the M028 install path and the M021 shape-guard. Confirmed against self-continue-drive.sh line 145 (ORCHESTRATOR_UNATTENDED=1 exported into the child). | No |
| D018 | Where does the production write-path/tool allowlist (the deny policy the hook reads) live? | Generated-per-run policy file the driver writes before spawn: the driver composes <milestone-dir>/.self-continue-scope-policy from (a) the committed framework-owned protected-surface manifest scripts/hooks/unattended-protected-surface.txt (read-only globs: SC/spec, roadmap, verification harness, scoring/ledger records) and (b) the runtime milestone work dir as the write-path allowlist, then exports ORCHESTRATOR_UNATTENDED_POLICY=<path> into the child; the hook reads that env var and fails closed (deny-all except read-class) when it is unset/unreadable under ORCHESTRATOR_UNATTENDED. | arch | M046/P05 | Per-run generation is required because the writable allowlist (the milestone work dir) is only known at spawn time, while the read-only protected surface is a stable committed contract; splitting committed-manifest + per-run-composition keeps the FR-20 protected surface auditable on disk while letting the write allowlist track the active unit. Mirrors the P01 probe DENY_PROBE_POLICY file format (allow_path/allow_tool/allow_bash) extended with a readonly_path directive. | Yes |
| D019 | What is the MCP tool-surface policy under --unattended? | Default-DENY all mcp__* tool calls unless an exact allow_tool directive names the tool; the empty default allowlist means every MCP call (Slack/Supabase/Gmail/etc.) is denied at PreToolUse before dispatch. Matches the P01 probe's proven mcp__* branch exactly. | arch | M046/P05 | MCP connections are the primary danger worktree filesystem isolation cannot contain (Problem Statement Gap 2); a default-deny-unless-allowlisted posture is the only fail-closed stance, and it is inside the SC-5 milestone-blocking gate (the MCP vector is a first-class blocked case, not a non-blocking example). | No |
| D020 | #Q-2: How is --yes broadening surfaced when orchestrator:do merges into orchestrator:auto? | --yes keeps its existing NARROW meaning (skip the single attended confirmation prompt — the Tier-A+ P02 approval prompt on the one-shot path, or the M029 preflight confirm on the Tier-C loop path); it does NOT broaden. --unattended (shipped P04) is the sole explicit gate for the unattended/destructive-approval envelope, governed by the FR-13 driver-level fail-closed caps. FR-5's broadening branch is therefore NOT triggered — no alias/longer-window/notice needed beyond documenting the boundary in commands/auto.md and commands/do.md. | arch | M046/P03 | do-entry.sh --yes only forwards to route-to-dispatch.sh's inter-stage approval prompt; it never touches the loop or any unattended authority. P04 already ships --unattended as the explicit envelope opt-in with driver-level fail-closed caps (FR-13, no disable flag per CON-4), so --yes has no destructive authority to broaden into. Keeping --yes narrow avoids a silent semantic change (FR-5) and preserves SC-2 byte-parity for existing do callers. | Yes — if a future flag consolidates confirm-skip and unattended-opt-in, re-open with an explicit alias/notice per FR-5. |
| D021 | #Q-3: How long is the orchestrator:do deprecation runway before shim removal, and what release gates it? | The do-shim ships as a deprecation shim in the M046 release and is retained through AT LEAST the next published minor release; shim removal is gated NO EARLIER than one published release after the deprecation ships. The one-line deprecation notice names the concrete target-removal version. Removal is a separate future change, explicitly NOT part of M046. | scope | M046/P03 | Installed consumers migrate via the orchestrator:update re-stage path (FR-4); giving at least one full published release with the shim + notice present lets scripted do callers observe the notice and migrate before the command disappears, honoring CON-3 default-off preservation. Naming the target version in the notice makes the runway operator-legible. | Yes — operator may extend the runway if consumer telemetry shows lingering do usage at the removal gate. |
