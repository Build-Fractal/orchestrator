...
2. **M014 — Spec Management + Comment→Workflow Automation** *(scope extended 2026-04-22 per D016)* — original: classify/auto-apply wiki+GH comments; invokes the M011/P07 conversus adapter for ambiguous-comment triage. **Extension**: port spec-kit's `specify` shape as native `orchestrator:specify` (CC-first, portable, treating spec-kit template as I/O contract); add conversus-suggestion logic so complex specs auto-propose red-blue pressure-test (with optional spec-decomposition-before-conversus for very large specs); dual-write `AGENTS.md` alongside `CLAUDE.md` so Codex stays fed without a separate milestone.
3. **M020 — Knowledge Layer Maturation** *(promoted 2026-04-21 per D013)* — review-state model, query/search surface for dispatch-callable knowledge, A6 Jaccard clustering in consolidate, full A1/A2/A3 workflow. Positioned before M024 so the router can reason over mature knowledge.
4. **M024 — Universal Intake & Routing** *(new, promoted 2026-04-22 per D016)* — extend `orchestrator:evaluate` to input-agnostic (idea/paragraph/fragment/spec/empty + Q&A); emit reviewable proposal artifact at `.orchestrator/intake/<id>/proposal.md` covering six axes (input shape, scope tier, decomposition, design gate, conversus gate, intensity); degenerate fast-path auto-proceeds if Tier A + Quick + no conversus + no design, approval-gated otherwise. Positioned post-M020 so it dogfoods through remaining milestones. Design gate degrades gracefully pre-M023 ("design walkthrough lands in M023; author DESIGN.md manually or skip").
5. **M019 Tier 2/3** — rollup + `orchestrator:cost`, then full polished surface — lands after M024 once ~3 milestones of Tier 1 data reveal what the polished surface should actually expose. Optional: fold a single Codex smoke-probe telemetry point here for drift detection ahead of M009.
6. **M018 — Context Compression Layer** *(sketch, see D008 + D010 four-tier reframing)* — caveman-style token compression as a pipeline stage. Phase outline drafted but not yet planned in detail; full planning happens closer to kickoff after M019 Tier 1 reveals which artifacts dominate token spend.
7. **M023 — Design Layer** *(new, promoted 2026-04-22 per D016)* — `orchestrator:design` command: conversus spawns N design-personality agents in parallel, each produces a DESIGN.md draft + working coded prototype in user's stack; side-by-side comparison; user picks (or arbitrates merges); canonical DESIGN.md lands at repo root and auto-injects into payloads for ui-tagged phases. Renderer adapter interface shaped as **MCP clients** (runtime-agnostic; Stitch/v0/Figma consumed as they expose MCP). Stage-1-only scope — high-fidelity stakeholder-mockup expansion deferred. Positioned pre-M009 because this repo has no UI to dogfood design against.
8. **M009 — Launch & Ecosystem** *(scope extended 2026-04-22 per D016)* — README, examples, contributor pipeline, release infra. **Extension**: runtime-parity audit as launch gate — dogfood every `orchestrator:*` command end-to-end on CC + Codex + Cursor, consume the `RUNTIME-ASSUMPTIONS.md` registry accumulated during M013–M018, fix parity bugs before external eyes.
9. **M010 — Cloud Dispatch** *(posture adjusted 2026-04-22 per D016)* — Managed Agents primary adapter + Codex Cloud stubbed adapter (proves abstraction without full feature parity). Full Codex Cloud as demand-driven fast-follow rather than launch-critical. Pull Codex Cloud forward if external demand skews Codex at launch.
