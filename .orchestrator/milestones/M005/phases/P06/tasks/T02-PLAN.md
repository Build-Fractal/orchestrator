...
2. **`scripts/diagnostics/check-run-ids.sh`** — Scans recent JSONL log entries for the `run_id` field. Pre-M004 entries may lack this field (legacy). The check focuses on entries from the current or most recent milestone. Emits `DOCTOR:RUNIDS status=<ok|warn> with_id=N without_id=N`.
