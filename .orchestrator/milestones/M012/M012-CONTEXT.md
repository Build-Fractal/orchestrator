...
- M012 ships **1 of 3** D011 criteria → **M020 is promoted** as a committed milestone (per `.orchestrator/DECISIONS.md` D011: ≤1 of 3 → promote).
...
- Mitigation: a documented one-time remap script (`scripts/diagnostics/giscus-remap.sh` or equivalent) transfers threads from the old path to the new path when a consolidation runs. The script ships in the same phase as the Giscus integration so it is ready when M011's first consolidation needs it.
...
- Comment classification, triage, or auto-application — **M014 scope**.
- GitHub Issues / Projects / Milestones sync — **M013 scope**.
...
- Pricing / usage / cost analytics surfaced in the wiki — **M019 Tier 2/3 scope**.
...
- **Constitution VIII (Bash 3.2 compatibility).** Shipped helper scripts (link checker, Giscus smoke test, remap script, deploy wrapper if any) are Bash 3.2 compatible. No `declare -A`. No compound bash in agent-facing content (M016/M021 anti-pattern linter applies).
