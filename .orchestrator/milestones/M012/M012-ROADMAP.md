...
- [x] **P01**: Wiki scaffold & content rendering — "Running `mkdocs serve` from the repo root renders every in-scope `.orchestrator/**.md` artifact at localhost with a navigable Material sidebar."
  ...
  - Boundary Map:
    ...
    - Consumes:
      - `.orchestrator/memory/constitution.md`, `.orchestrator/DECISIONS.md`, `.orchestrator/KNOWLEDGE.md`, `.orchestrator/milestone-summary.md`
...
- [x] **P02**: Cross-link rewriting, MEM resolution, link-checker — "Clicking an internal markdown link on the rendered site navigates to the rendered target (including KNOWLEDGE MEM entries like `MEM-0012`); the build-time link-checker reports zero broken in-scope links."
  ...
  - Boundary Map:
    - Produces:
      ...
      - The D011 mechanical evaluation for M012 runs at the close of this phase: 1 of 3 criteria shipped (cross-refs ✓, review-state ✗, query surface ✗) → M020 is promoted per `.orchestrator/DECISIONS.md` D011 and noted in P02-SUMMARY.md
    - Consumes:
      - `wiki/mkdocs.yml`, `wiki/docs/**` (P01)
      - `.orchestrator/KNOWLEDGE.md` MEM entries (as cross-ref targets)
...
- **Bash 3.2 compatibility (Constitution VIII)** — P02, P03. P02 establishes the pattern for Bash-3.2-safe diagnostic scripts (`wiki-link-check.sh`); P03 follows (`wiki-giscus-smoke.sh`, `wiki-giscus-remap.sh`). No `declare -A`; no compound bash in agent-facing content (M016/M021 linter applies).
...
- **D011 trigger evaluation runs at the close of P02** — The mechanical evaluation per `.orchestrator/DECISIONS.md` D011 counts how many of the three criteria (cross-refs to `knowledge/**/MEM*.md`, reviewed/unreviewed state, dispatch-callable query surface) shipped. M012 ships 1 (cross-refs, in P02). Result: **M020 promoted** as a committed milestone. This outcome is recorded in P02-SUMMARY.md and triggers a roadmap update post-M012 to position M020 between M014 and M019 Tier 2/3 per D011's framing.
