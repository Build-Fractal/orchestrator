...
- M012/P04 established the `pending`-sentinel pattern for first-deploy operator handoff (see `.orchestrator/DECISIONS.md` D012 / the `DEPLOY-RECORD.md` convention). This task inherits that discipline for `github.json`: the sentinel string is the literal word `pending` (lowercase, unquoted in JSON string form: `"pending"`).
...
- No jq hard dependency: JSON parse in the verify gate prefers `python3` and falls back to `jq` only if present; `SKIP:` when neither (graceful-absent-tool pattern from M012).
