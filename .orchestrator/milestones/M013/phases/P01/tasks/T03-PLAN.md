...
2. Parses as valid YAML (prefer `python3 -c 'import yaml,sys;yaml.safe_load(open(sys.argv[1]))'`; fall back to `yq` if present; `SKIP:` if neither, following M012 graceful-tool-absent pattern).
