...
- `scripts/verify/anti-pattern-lint.sh` (M016/M021 invariant) — T07 gate consumes.
...
- **Constitution XII (Hook Isolation)** + **Constitution IX (Bash 3.2)**: no wrappers that change child process env; exit non-zero on preflight failure — never fallback to graceful degradation (M007 principle).
