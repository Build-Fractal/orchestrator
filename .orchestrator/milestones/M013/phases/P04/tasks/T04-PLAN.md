...
- T01 has landed the sync fixture tree (this task reuses the M021 prompt-corpus replay but does not itself read T01 helpers).
...
- **M008 runtime adapter interface contract preserved**: `--probe`, `--register`, `--hook-config`, `--dry-run` all remain; only the `--hook-config` JSON content changes.
