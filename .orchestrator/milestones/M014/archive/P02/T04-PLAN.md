...
- `scripts/diagnostics/check-docs.sh` (from M006) is the existing docs-completeness diagnostic. Its current output shape on exit 0 is `DOCTOR:DOCS status=ok found=<N> total=<N>`; on failure it exits 1 with missing-file listings.
...
- `.orchestrator/milestones/M014/phases/P02/WRITE-SITES.md` (from T01) — names the two marker regions drift detection scans for.
