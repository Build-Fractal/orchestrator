...
3. Six new verifiers under scripts/verify/m014-p03-*.sh:
   ...
   - zero-prompts: invokes comments.sh classify --yes under hermetic scratch with the four-class T04 fixture and a non-existent COMMENTS_ADAPTER, captures stdout+stderr, asserts zero matches against the M021 prompt-corpus INPUT lines + an interactive-prompt regex (\(y/n\), \[y/N\], read -p, etc.)
