...
- Probe runs against `specs/024-spec-management-extended/spec.md` with `CLAUDE_CODE_RUNTIME=0 SPEC_COMPLEXITY_PROBE_LLM=0` → emits `probe=above-threshold reason=fr_count>=15` (heuristic-only; M024 has ~20 FRs); structured fields all present on stderr; `contradiction_signals=0`.
...
- Probe emits `probe=below-threshold` on M016 (hardening-spec exception) under non-CC runtime
