...
- `.orchestrator/execution-log.jsonl` is the M019 Tier 1 append target. JSONL records for `unit_close` events follow the shape:
  ```
...
(macOS `date` does not support `%N` for milliseconds in Bash 3.2; using second-resolution with `000` suffix is the accepted project precedent — see M016/M021 event emitters.)
...
- `.orchestrator/milestones/M014/phases/P02/WRITE-SITES.md` (from T01) — lists consolidate-artifacts.sh as a `recent-changes` call site.
