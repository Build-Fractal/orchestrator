...
- **Env-overridable sub-script paths** — `COMMENTS_FETCH`/`COMMENTS_CLASSIFY`/`COMMENTS_ADAPTER`/`COMMENTS_UAT_INGEST`/`COMMENTS_APPLY`/`COMMENTS_REJECT`/`COMMENTS_TRIAGE` extend the T01 `ORCHESTRATOR_PROJECT_ROOT` hermetic pattern to every fan-out point; lets verifiers stub the adapter + M013 UAT ingest without modifying real binaries (D007 reuse preservation).
...
- **SC-7 zero-approval-prompt**: `comments classify --yes` under hermetic scratch produces zero approval-prompt-shaped strings (M021 corpus cross-check).
