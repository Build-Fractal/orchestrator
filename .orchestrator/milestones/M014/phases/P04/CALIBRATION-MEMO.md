...
Six specs shipped under the post-M011 spec-kit regime. Numeric measurements were re-taken at T01 dispatch time using the heuristic patterns specified in the planner (FR count via `grep -cE '^- \*\*FR-[0-9]+|^### FR-[0-9]+|^\*\*FR-[0-9]+'`, user-story count via `grep -cE '^### User (Story|Scenario)'`, tokens via `wc -w`, TODO count via `grep -cE '<TODO'`).

| Spec | FR | US | Tok | TODO | Label | Reason |
|---|---|---|---|---|---|---|
| M011 (011-spec-management) | 16 | 5 | 2392 | 0 | above | `fr_count>=15` |
| M013 (023-github-native-integration) | 18 | 6 | 7851 | 0 | above | `user_story_count>=5` |
| M016 (016-autonomous-hardening) | 0 | 3 | 1218 | 0 | below | hardening-spec exception |
| M021 (021-autonomous-hardening-v2) | 0 | 5 | 2430 | 0 | below | hardening-spec exception |
| M022 (022-spec-wiki) | 10 | 5 | 3613 | 0 | above | `user_story_count>=5` |
| M024 (024-spec-management-extended) | 20 | 5 | 11083 | 14 | above | `fr_count>=15` |
...
- `.orchestrator/DECISIONS.md` D014 — M013 conversus deliberation precedent (above-threshold firing on high-FR/high-US spec validated the gate intent)
