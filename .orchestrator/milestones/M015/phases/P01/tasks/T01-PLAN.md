...
Delete the spec-kit extension host artifacts from the repo. This is a hard delete — no rename, no archive, no compat shim (per M007 no-graceful-degradation discipline). After this task, the repo no longer registers itself as a spec-kit extension and no longer ships the dogfooded spec-kit slash commands or templates.
