...
5. Spot-check one migrated artifact per major category — this is a sanity check, not a verify-script gate. Each of these commands should exit 0:
   ...
   ```
   test -f .orchestrator/KNOWLEDGE.md
   test -f .orchestrator/DECISIONS.md
   test -f .orchestrator/execution-log.jsonl
   test -d .orchestrator/milestones/M001
   test -d .orchestrator/milestones/M015
   test -f .orchestrator/milestones/M015/M015-ROADMAP.md
   test -f .orchestrator/milestones/M015/phases/P01/P01-SUMMARY.md
   ```
...
- `.orchestrator/config.yml`, `.orchestrator/KNOWLEDGE.md`, `.orchestrator/DECISIONS.md`, `.orchestrator/execution-log.jsonl`, and `.orchestrator/milestones/` all present.
...
- `.specify/orchestrator/config.yml` — already has `state_root: ".orchestrator"` (pre-declared by M008). The migration doesn't need to rewrite it.
