...
- **P02 — Knowledge-aware filter** (US-2). First production code: extends `scripts/dispatch/build-context.sh` to read M020 `status:` and apply the configured drop-list. Zero-LLM. Emits `payload_filter` JSONL.
...
FR-10 (additive `dispatch_usage` / `unit_close` fields) and FR-11 (M027 surface extensions) are bundled into one phase rather than threaded incrementally into P02–P05. Rationale:
...
- M019 emitter schema extensions (additive, back-compat) for filter savings, T1/T2/T3 savings, T1/T3 invocation counts.
