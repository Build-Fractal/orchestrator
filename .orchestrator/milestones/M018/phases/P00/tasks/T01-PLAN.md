...
2. **Wire dispatch-interface.sh into build-context's caller**: identify what calls `build-context.sh` and route the result through `dispatch-interface.sh`. Higher-touch; risks regressing other M019 invariants.
