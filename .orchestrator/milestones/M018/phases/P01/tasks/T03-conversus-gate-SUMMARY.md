...
Token-economy decision: chose to address only the two P0 mitigations rather than the full P0+P1+P2 set, then re-run gate. Rationale: minimum-fix surface area maximizes PASS odds on retry by reducing red-advocate's attack surface; P1 mitigations MIT-06/07 require empirical validation work (tier3-on-MEM-heavy probe; tier2 → tier3 sequential simulation) that's substantive engineering, not regex tweaks. Followup deferrals are documented in grammar Open Questions with named risk-rationale per item. Pattern is reusable for future conversus-gated phases (M013/M014/M023/M024).
