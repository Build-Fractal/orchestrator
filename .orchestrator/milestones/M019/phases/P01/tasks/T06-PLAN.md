...
2. `scripts/verify/m019-p01-no-pre-p00-emission.sh` — SC-12 hard ordering: asserts no emitter record exists in any post-M011 milestone's `execution-log.jsonl` with a timestamp earlier than `2026-04-18T02:21:28Z` (P00 SUMMARY `completed_at`).
...
This is also the task that walks the full emitter path end-to-end on a real (not fixture) milestone to produce the first post-P00 records for M012 dogfooding — but only after every gate is green.
...
- `.orchestrator/milestones/M019/phases/P00/P00-SUMMARY.md` — `completed_at: "2026-04-18T02:21:28Z"` is the SC-12 epoch.
