...
- **Cross-phase coordination**: the spec explicitly holds schema authority over `knowledge/spec/**` and `knowledge/**/MEM*.md` (FR-9, Knowledge-Layer Boundary section) on behalf of downstream milestones (M024 universal intake, M019 Tier 2+3 observability, M018 context compression). This boundary requires roadmap decomposition so schema-evolution changes land in the correct phase.
...
- **Complex dependency graph**: the spec depends on M012 (closed — spec wiki), M019 Tier 1 (closed — observability emitter), M021 (closed — shape-guard), and a pre-M020 minimum-viable `graduate.sh` subset (per D011/D013). Downstream consumers span M024, M019 Tier 2+3, M018, and M009.
