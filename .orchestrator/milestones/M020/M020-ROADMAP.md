...
- [x] **P01**: Knowledge schema foundation + min-viable graduate.sh + Jaccard validation — "Running `bash scripts/knowledge/graduate.sh --rationale 'test' <entry-id>` flips an entry's `status:` from `candidate` to `graduated`, and `bash scripts/knowledge/lib/jaccard.sh validate knowledge/` writes a validation report at `.orchestrator/milestones/M020/phases/P01/jaccard-validation-report.md` confirming the 0.7 threshold + CON-5 feature vector against the live knowledge tree."
  - Risk: high
  - Depends: none
  - Boundary Map:
    - Produces:
...
- [x] **P03**: Candidate→graduate workflow extension + decision history + schema-authority lint — "Running `bash scripts/knowledge/graduate.sh --cluster C1 --rationale 'merge — same assertion'` against a three-entry candidate cluster flips one entry to `graduated`, two to `archived` with `archived_into:` back-references, and appends a `decision_history:` block containing rationale + ISO-8601 timestamp + operator email to all three entries; `bash scripts/verify/knowledge-schema-lint.sh` exits non-zero on a fixture introducing an unauthorized frontmatter field."
  ...
  - Boundary Map:
    - Produces:
      ...
      - JSONL emit: `knowledge_graduate`, `knowledge_archive` records (consumed by M019 Tier 2+3 downstream)
...
2. **P02 + P03 — can execute concurrently** — both depend only on P01. Both high risk; P02 unblocks M024 query-surface dependency, P03 ships the schema-authority lint that all subsequent phases verify against. Parallel execution maximizes throughput on the critical path.
