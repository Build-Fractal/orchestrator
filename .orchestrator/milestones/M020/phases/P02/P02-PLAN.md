...
No files under `knowledge/**` are touched; no files under `.orchestrator/memory/` or `.orchestrator/DECISIONS.md` are touched (no schema evolution in P02 — schema authority work landed in P01 per D024). FR-8 + CON-1 + SC-7 demand `git status knowledge/` clean post-invocation; verifier `m020-p02-query-side-effect-free.sh` enforces this directly.
