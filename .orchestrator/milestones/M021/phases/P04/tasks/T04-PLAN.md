...
- `.orchestrator/DECISIONS.md` contains rows D001..D011 (current head-of-file state) — verify before editing.
...
- `.orchestrator/DECISIONS.md` — GitHub-style markdown table with header row plus 11 data rows (D001..D011). Column order: `#`, `When`, `Scope`, `Decision`, `Choice`, `Rationale`, `Revisable?`. Append-only; new rows go at the end.
...
The M021 roadmap and M021-CONTEXT.md (authored 2026-04-17) anticipated that the M021-before-M019 reorder decision would land as `D010`. Between roadmap authoring and P04 execution, `D010` (M018 framing amendment per article synthesis) and `D011` (M020 trigger criteria at M012/P02 close) landed in `.orchestrator/DECISIONS.md`. P04 therefore logs the M021 reorder as **`D012`** — the next available sequential ID — with a parenthetical note in the Rationale column documenting the ID reassignment for audit continuity.
...
1. **T04.a — Append D012 to `.orchestrator/DECISIONS.md`**: one new row documenting the M021-before-M019 reorder. Every pre-existing row (D001..D011) remains byte-identical.
...
### Step 1 (T04.a): Append D012 row to `.orchestrator/DECISIONS.md`
...
- `.orchestrator/DECISIONS.md` grows by exactly 1 line (one table row).
- `ANTIPATTERNS.md` grows by approximately 5 × 4 = 20 lines (5 entries × ~4 lines of Cross-Refs content each). Envelope: ≥5, ≤40 new lines.

## Must-Haves

- `.orchestrator/DECISIONS.md` contains a new D012 row with required substrings: `D012`, `sequencing`, `M019`, `zero-prompt`.
...
- `.orchestrator/DECISIONS.md` — 11 data rows (D001..D011); T04 appends D012.
...
- `.orchestrator/DECISIONS.md` — 12 data rows (D001..D012); file size grows by approximately 1 line's worth of content.
