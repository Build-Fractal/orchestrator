...
- **P01 — Foundation**: input-shape detector (`shape-detect.sh`), intake-id allocator with M020 schema-authority backing, six-axis proposal template (`templates/intake-proposal.md`), and the proposal emitter stub (`proposal-emit.sh`). Establishes the `.orchestrator/intake/<id>/` write-confinement boundary (SB-3) and pins the proposal frontmatter at `schema_version: "1.0"` (AD-3).
- **P02 — Spec-path backcompat**: spec deep-classifier (`spec-shape-classify.sh`) preserves byte-compatible legacy evaluation output (FR-6) while emitting a parallel proposal with `input_shape: spec`. M014 interim-manifest reads pass through `m014-manifest-read.sh`.
...
- **M013 GitHub integration**: the proposal artifact at `.orchestrator/intake/<id>/proposal.md` is naturally addressable for future PR-comment workflows; no integration shipped this milestone.
