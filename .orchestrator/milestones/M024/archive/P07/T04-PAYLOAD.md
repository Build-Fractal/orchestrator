...
# Dispatch Context -- T04 (Phase P07, Milestone M024)
## Manifest
...
| Task Plan | 1123-1570 | ~6300 | required |
| State Context | 1572-1578 | ~100 | required |
| First-Turn Completeness | 1580-1632 | ~1000 | required |
| **Total** | | **~22700** | |
...
8. **`.orchestrator/DECISIONS.md`** — append a single D-row entry documenting the `pending_design_authored_manually` schema addition under MEM031 / D024 schema-authority handshake. The entry names M020 as the authority-holder, M024 as the consumer, the field semantics (transient flag, never carries semantic value beyond "operator is authoring DESIGN.md"), and the closed-enum default (`false` on emit; `true` only between manual-branch first-invoke and follow-up).
...
### Step 8 — Append D-row to `.orchestrator/DECISIONS.md`

Locate the most-recent D-row in `.orchestrator/DECISIONS.md` (likely D024 from M020/P01 or higher if M020/P02+ landed). Append a new D-row immediately after the latest one. The text:
...
- `.orchestrator/DECISIONS.md` carries a new D-row documenting the `pending_design_authored_manually` schema addition.
...
- `.orchestrator/DECISIONS.md` — appended to (not modified in place).
...
All thirteen P07 verifies pass; the P07 suite exits 0; P03 and P06 suites remain green; `commands/evaluate.md` Pre-M023 section is updated to "wired in P07"; `.orchestrator/DECISIONS.md` carries the new D-row.
...
- .orchestrator/DECISIONS.md (modify — append D-row for the new transient frontmatter key under MEM031 / D024 schema-authority handshake)
