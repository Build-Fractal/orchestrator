...
- `tests/fixtures/m014-interim-manifest-keys.txt` — 6-key M014 interim manifest fixture (`schema_version`, `type`, `feature_slug`, `created_at`, `status`, `milestone`).
...
- `scripts/intake/proposal-emit.sh` (T04 deliverable) — added computation + substitution for all three new keys:
  ...
  - `milestone`: parsed from `.orchestrator/milestone-summary.md` if present (matches both `^**Active milestone**:` and `^**Out-of-band active milestone**:` shapes); otherwise emits `null`. (For the current repo, the active marker is `Out-of-band active milestone` so `M026` is emitted.)
