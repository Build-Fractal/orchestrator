...
- `knowledge/lessons/MEM026.md` (min 20 lines, contains "d33b8a7") — create (adjust MEM number at task time if KNOWLEDGE-INDEX advanced)
...
- `specs/021-github-installer-coexistence/spec.md` → `.orchestrator/milestones/M025/M025-ROADMAP.md` (spec authoritatively drives this phase's scope)
- `.orchestrator/milestones/M013/phases/P04/P04-SUMMARY.md` → this plan (regression origin; read-only context)
...
6. `scripts/verify/m025-p01-knowledge-entries.sh` — asserts `knowledge/lessons/MEM026.md` (or whichever number T04 picks) exists and contains `d33b8a7` + `M013/P04/T04`; asserts a matching pattern entry exists; asserts `KNOWLEDGE-INDEX.md` lists both.
...
**Update CLAUDE.md status line.** In the `## Project Status` section of CLAUDE.md, edit the line naming completed milestones to add "M025 (installer coexistence, 2026-04-23)" alongside the existing entries. Update the Forward Roadmap paragraph to remove the stale "M013 P04 is the only phase left in M013" sentence.
