...
Threshold defaults (cost multiplier, retry / pass-rate floor) require sampling current M012–M026 data to know what "normal" looks like. Plan-phase fires the measurement and pins the defaults in the phase plan. Until then, the spec carries proposed defaults (3× median; `retry_count > 2 OR verification_pass_rate < 0.5`) as guidance, not contract.
...
- M018 (Context Compression) — M027 surfaces the data; M018 acts on it.
- Schema changes to Tier 1 records.
- Backfilling pre-M019 records.
...
### Knowledge-Layer Boundary (M027 vs M025)
