...
- **Runtime portability for the new command**: `orchestrator:cost` must register cleanly on Claude Code, Codex CLI, and Cursor via the existing packaging adapters from M015 + M025. The command is new but the adapter surface is established; still warrants its own dispatch.
