...
# Dispatch Context -- T03-cost-command (Phase P01, Milestone M027)
## Manifest
...
| Task Plan | 1023-1212 | ~3500 | required |
| State Context | 1214-1220 | ~100 | required |
| First-Turn Completeness | 1222-1263 | ~800 | required |
| **Total** | | **~18600** | |
...
```bash
bash scripts/verify/m027-p01-cost-command-shape.sh
```

The above (T04 deliverable; placeholder until T04 lands — at T03 verification time the operator runs T03's own truth-validation manually by `grep`-ing the file). The phase-level `bash scripts/verify/check-must-haves.sh .orchestrator/milestones/M027/phases/P01` runs at the phase boundary, not at T03 task verification.

T03's own immediately-runnable check is:

```bash
bash scripts/verify/m027-p01-cost-command-shape.sh
```

This verifier ships in T04. Until T04 lands, T03's expected post-state is a syntactically valid `commands/cost.md` + skill stub + manifest update; T04's verifier formalizes the gates.
