...
- **Sibling-finding fold**: Findings A (hook portability) and F (M025 hook-shim follow-up) share the installer + adapter surface and ship together in P02. Splitting them creates an integration gap (bare-name commands pointing at runtime-stable paths, or hook portability without working lifecycle scripts) — verification only closes when both ship in the same PR.
...
- **Strict-superset classifier guarantee**: SC-8 + FR-22 + CON-7 require zero regressions on M021's 21 corpus entries. Every classifier change must preserve prior verdicts.
