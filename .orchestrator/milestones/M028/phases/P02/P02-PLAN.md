...
- The shape-guard hook self-conforms to its own classifier output under AP-009 (no compound chain exceeding 2 connectors anywhere in its body). Verified by sourcing the M021 classifier, scanning the hook body line-by-line, and asserting `classify_command` returns ALLOW for every non-comment non-blank line. Satisfies CON-3 + FR-21 (P02 half — P03's `finding-G-self-conformance.sh` verifies via the M028 classifier; P02 verifier here uses M021 classifier as the day-one floor).
