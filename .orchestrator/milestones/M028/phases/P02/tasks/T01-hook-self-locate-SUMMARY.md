...
**Self-conformance verifier scope: resolution block, not entire file.** The plan's step 5 specifies "reads `scripts/hooks/pre-bash-shape-guard.sh` line-by-line ... For each line, skip if blank or starts with `#`. Otherwise call `classify_command "$line"`. ... If the verdict starts with `REJECT`, emit FAIL and exit 1." Empirical scan of the unmodified M021 hook against the M021 classifier returns 14 `reject:*` verdicts on pre-existing lines:
