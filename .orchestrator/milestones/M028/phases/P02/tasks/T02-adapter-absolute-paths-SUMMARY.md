...
Every leaf carries `"_orchestrator_managed": true` (M025 invariant; uninstall-cascade key). The comment block was rewritten to document the new shape, the rename rationale, the heredoc-quote discipline, and the runtime-stable-hooks-dir contract. The four `TODO(M025+)` deferral markers are preserved.
