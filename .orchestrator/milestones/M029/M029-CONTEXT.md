...
**Decision**: The `orchestrator:auto` preflight summary at Standard / Full intensity respects M031's `auto_proceed: true` config flag and an explicit `--yes` flag, in priority order:
...
**Decision**: Amend SC-8 to use the actual M027 surface; **do not** extend M027 (that would breach the knowledge-layer boundary in CON-3 and the M027-is-closed assumption in A-1).
...
- Spec `033-reference-corpus-ingest/spec.md` (the only existing multi-milestone spec, currently `milestone: "M036 (split: M036a pre-launch, M036b post-launch — see Amendment Record)"`) is the migration test case — it gets `milestones: [M036a, M036b]` added, but the migration is **out of scope for M029**. M029 only needs to handle the schema; spec 033's migration lands when M036b enters planning.
...
- **No** extension to KNOWLEDGE.md schema (M020 owns)
- **No** changes to M027 surfaces (`metrics-rollup.sh`, `efficiency-footer.sh`, `predictive-surface.sh`) — M029 is a read-only consumer
- **No** changes to M013's GitHub sidecar schema (FR-11/FR-12 explicitly cut deeper integration)
...
- M033 friendly-tester pass deadline ≤ 2026-05-12 per launch-sequencing-amendment Q-1 (`tests/m033-acceptance/friendly-tester-pass/protocol.md`) — runs alongside M029; not a dependency.
