...
1. **`tests/m029-acceptance/p02-acceptance-battery.sh`** — wraps the four P02 SC acceptance scripts (SC-5, SC-6, SC-13, SC-14) and emits `BATTERY: pass=N fail=M` per the M030/M031 acceptance-battery convention. Re-used by `validate-milestone.sh M029` (P03 deliverable) alongside the P01 + P03 batteries.
...
1. **Author `tests/m029-acceptance/p02-acceptance-battery.sh`** (≥20 lines, executable, AD-19 single-script-file shape, bash 3.2). The battery:
   ...
   #
   # SC coverage: SC-5 (where mixed-state golden), SC-6 (pre-M019 cost suppression),
...
4. **Author `tools/verify/m029-p02-scope-guard.sh`** (≥40 lines, executable, AD-19, bash 3.2). Pattern after `m029-p01-scope-guard.sh`:
   ...
   - Defines the **denylist** (paths that must NEVER be touched by P02):
     - `commands/auto.md`, `commands/init.md`, `commands/dispatch.md` (M031/M033 surfaces)
     - `scripts/lifecycle/auto-loop.sh` (Principle XV — M029 does not touch the auto loop)
     - `scripts/diagnostics/metrics-rollup.sh`, `scripts/diagnostics/efficiency-footer.sh`, `scripts/dispatch/predictive-surface.sh` (M027 read-only consumer; CON-7 / AD-8)
     - `.orchestrator/integrations/github.json` (CON-4 / FR-11)
     - `.orchestrator/KNOWLEDGE.md`, `.orchestrator/DECISIONS.md` (M020 schema authority; CON-7)
...
- A summary file at `.orchestrator/milestones/M029/phases/P02/tasks/T05-phase-close-gates-SUMMARY.md` documents the deliverables.
