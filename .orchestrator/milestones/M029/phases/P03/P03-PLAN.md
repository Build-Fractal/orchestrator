...
- The P03 scope-guard verifier `tools/verify/m029-p03-scope-guard.sh` exists, is executable, captures `git status --porcelain=v1`, classifies each touched path against the P03 allowlist + denylist, and exits 0 unless any deny hit appears. Allowlist enumerates exactly the `Files Likely Touched` paths declared below; denylist enumerates `commands/init.md` (M033), `scripts/lifecycle/auto-loop.sh` (Principle XV), `scripts/diagnostics/metrics-rollup.sh`, `scripts/diagnostics/efficiency-footer.sh` (M027 read-only consumer per CON-7/AD-8), `.orchestrator/integrations/github.json` (CON-4/FR-11), `.orchestrator/KNOWLEDGE.md`, `.orchestrator/DECISIONS.md` (M020 schema authority per CON-7). `WARN:` for unclassified, FAIL only on deny hits — P01/P02 precedent.
...
- The closure ceremony fires: `.orchestrator/milestones/M029/M029-VALIDATED` marker exists; `.orchestrator/milestones/M029/M029-SUMMARY.md` exists with the canonical milestone-summary frontmatter (15 fields per write-summary.sh task-mode usage example expanded shape) + body summarizing what was built, what was deferred, and what the post-close handoff is (M035 P00+P01 ergonomic prep before launch); the milestone-grain `unit_close` event is appended to `.orchestrator/milestones/M029/execution-log.jsonl` per the M019 emitter convention (consumes existing emitter; produces no new event type per CON-7).
...
- `.orchestrator/milestones/M029/M029-SUMMARY.md` (create; min 80 lines, contains "milestone: \"M029\"", contains "completed_at:", contains "verification_result: \"pass\"")
...
- `.orchestrator/milestones/M029/M029-SUMMARY.md` (create)
