...
1. **`commands/auto.md` `## Preflight Summary` section** (additive modification): a new H2 section prepended above the existing first H2 (or at a documented position near the top after the YAML frontmatter + H1 title), documenting the FR-9 + AD-3 + AD-4 contract:
   ...
   - **AD-3 non-interactive policy** (priority order):
     ...
     2. `auto_proceed: true` in config (M031 default) → emit preflight on stderr, do not prompt, proceed.
...
- **CON-7 / AD-8 knowledge-layer-boundary**: T02 introduces NO new JSONL events, NO M020 changes, NO M027 surface changes. The skill doc cross-references existing M027 surfaces; it does NOT extend them.
...
- A summary file at `.orchestrator/milestones/M029/phases/P03/tasks/T02-auto-preflight-summary-SUMMARY.md` documents the deliverables.
