...
- **No M030 scope expansion beyond the 8 findings**: M032 just absorbed wiki-distribution scope; pre-launch work is growing. Plan-phase MUST resist adding work beyond what these decisions require. Specifically: no per-task LLM-driven re-classification, no auto-tuning of `cost_rates`, no second runtime adapter beyond CC.
- **Sequencing assumption**: M028 (autonomous hardening v3) ships before M030 — M028 stabilizes autonomous runs that M030's shadow corpus depends on for clean signal (A-5 in spec). Plan-phase MUST verify M028 closure before authoring P01.
