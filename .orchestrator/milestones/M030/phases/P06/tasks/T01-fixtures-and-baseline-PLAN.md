...
- `scripts/diagnostics/check-anomalies.sh` exists in its post-M027/M018 form (~290 lines; sourceable + CLI; `set -u` only; reads `metrics-rollup.sh` task-grain rollup; emits a "Anomaly Detection (Tier 1 baseline)" text block to stdout; suppression matrix = 5 conditions; bash 3.2 compatible). Verified at plan-time by direct `find` + Read.
...
- **Project-owned-verifier-paths discipline (M032 Finding A)**: every new verifier lives under `tools/verify/` with a slug-bearing filename `p06-*`. None under `scripts/verify/`.
