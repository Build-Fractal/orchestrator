...
- `scripts/diagnostics/check-anomalies.sh` is in its post-M027/M018 form. Key surface T02 will amend:
...
- "anomaly surfaces through orchestrator:doctor per existing M027 conventions" — gated by `bash tools/verify/p06-doctor-surfaces-anomaly.sh`.
...
- `scripts/dispatch/dispatch-interface.sh` — Key API: emits `dispatch_usage` JSONL records via `_di_emit_dispatch_usage`. Shadow-on emit branch (gated by `M030_SHADOW_MODE=1 && CLAUDECODE=1`) carries 9 additive fields beyond M019 baseline. T02 appends `character` as a 10th additive field.
...
- **Project-owned-verifier-paths discipline (M032 Finding A)**: every new verifier lives under `tools/verify/p06-*`; none under `scripts/verify/`.
