...
- The dup-prevention test in SC-6 (`run_dup_prevention`) uses the **direct invocation approach** — invoking `ingest-codebase.sh` directly against a pre-seeded fixture rather than going through start.sh + migrate.sh. This separates FR-11 (migrate-routing) from FR-12 (dup-prevention) at the test layer; each is verified against the surface it controls. The compound test (start.sh → migrate.sh → ingest-codebase.sh end-to-end) is out of scope for SC-6 because it depends on M015's actual MEM-emission shape, which is not under M033's control.
