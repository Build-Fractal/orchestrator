...
- **D-T04-06 (battery min-80-line floor)**: The battery-shape verifier asserts a min-80-line floor on the runner. The minimal byte-equivalent of M030/M031 patterns came in at 67 lines. Padding the difference with comment text would be cosmetic; instead, the header gained a closed-enumeration roster comment block mapping each SC to its script path. The roster doubles as load-bearing traceability for the M033-VALIDATED gate's SC-by-SC pass-roster lookup (T05 will read this when synthesizing M033-SUMMARY.md).
