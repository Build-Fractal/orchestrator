...
- [x] **P03**: Cursor MCP review-gate server + RUNTIME-ASSUMPTIONS + byte-parity audit — "An interactive Cursor session renders a native elicitation form at a gate and captures the operator's accept; a headless cursor-agent run gets the deterministic decline mapped to the auto-mode policy." (US4, FR-14, FR-15; consolidates M009 FR-6 + FR-8) *(closed 2026-06-06: review-gate-mcp-server.sh bash+jq stdio elicitation/create server (pure transport, delegates to interactive-review.sh — AD-1) + non-clobbering merge-mcp-config.sh into .cursor/mcp.json (CON-6) + cursor.sh --mcp-config + install-cursor.sh Stage 3.6 + RA-M034-REVIEW-01/02/03 RUNTIME-ASSUMPTIONS rows + renderer-doc Cursor-MCP path + byte-parity audit (frozen-ts whole-file SHA) + ORCH_REVIEW_FIXED_TS seam; resolves PC-6 + #Q-5; verifies SC-6/SC-9; phase-suite PASS 4/4; check-must-haves 53 PASS/0 FAIL; M009 FR-7 deferred)*
  - Risk: medium
  - Depends: P02
  - Boundary Map:
