...
- T04 closed three loose ends in one task: (a) the M032 SC-5
  ...
  (b) D-RN-1 npm-name collision-check evidence captured at
  ...
  `D-RN-1: @build-fractal/orchestrator` per RENAME-PLAN.md fallback);
  ...
  unprefixed `p00-phase-suite.sh` shape silently clobbered M030's
  aggregator with M031's, and M031's with M036's; this filename never
  collides).
