...
- `.orchestrator/DECISIONS.md` — exists; uses
  ...
  file's tail).
- `references/RENAME-PLAN.md` — exists; provides D-RN-1..D-RN-7 source
...
- D-RN-1 = `@build-fractal/orchestrator` (P00 collision-check confirmed
  available; recorded in P00 SUMMARY).
- D-RN-2 = `Build-Fractal/orchestrator` (off-tree).
- D-RN-3 = `orchestrator:<cmd>` cohort prefix.
- D-RN-4 = `build-fractal/orchestrator` single-formula tap.
- D-RN-5 = `~/Sites/orchestrator`.
- D-RN-6 = migrate Claude memory dir.
- D-RN-7 = pre-rename tag `v0.9.X-final-spec-kit-name`.

**Off-tree operator pre-condition**: the pre-rename git tag (D-RN-7)
...
Author the foundational pieces of P01.5: (a) the `D-RN-1..D-RN-7`
decision block in `.orchestrator/DECISIONS.md` so downstream phases
...
1. **Read `.orchestrator/DECISIONS.md` tail to resolve next NNN**.
   Read the file with `bash scripts/util/read-range.sh
   .orchestrator/DECISIONS.md -1 -1` (or equivalent shape-guard-safe
   tail wrapper) to find the highest existing `#dr-code-NNN` anchor.
   Resolve `<NEXT_NNN>`, `<NEXT_NNN+1>`, ..., `<NEXT_NNN+6>` as the
   seven new anchors for D-RN-1..D-RN-7. Cross-reference any milestone
   logs to ensure no NNN collision.

2. **Append D-RN-1..D-RN-7 decision block to
   `.orchestrator/DECISIONS.md`**. Each decision gets its own heading
   per the existing dr-code-NNN convention. Body shape (per existing
   convention from `### Order of remaining milestones after M015
   { #dr-code-004 }`):

   ```markdown
   ---

   ### D-RN-1 — npm package name `@build-fractal/orchestrator` { #dr-code-<NEXT_NNN> }
   ...
   - **Choice**: npm publish target is `@build-fractal/orchestrator`
     ...
     binary name, and CLI command-cohort prefix downstream (D-RN-2 through D-RN-4).
   ...
   Repeat the shape for D-RN-2..D-RN-7 (one heading each, anchor
   ...
   - **D-RN-5** — Local clone path `~/Sites/orchestrator` (operator
   ...
   - **D-RN-6** — Migrate Claude memory dir
     ...
     working-dir path).
   - **D-RN-7** — Pre-rename version tag `v0.9.X-final-spec-kit-name`
...
7. **Author `tools/verify/m035-p015-decisions-block.sh`**. Verifier
   asserts the D-RN-1..D-RN-7 block landed in `.orchestrator/DECISIONS.md`:
...
- D-RN-1..D-RN-7 are recorded in `.orchestrator/DECISIONS.md` with
...
- `.orchestrator/DECISIONS.md` — existing register (extended, not
...
- **Operator advisory** for the pre-rename tag step: if the auto-loop
  ...
  `PAUSE: D-RN-7 pre-rename tag — operator runs \`git tag
...
- `.orchestrator/DECISIONS.md` carries seven new `### D-RN-N — …`
  headings (anchored per dr-code convention) recording D-RN-1..D-RN-7.
