...
- **`--uninstall` short-circuit** in all three installers now
  ...
  - `mode:copy|*` (default fallback): removes the regular file iff
    `[ -f "$f" ]` holds — preserves existing M025 behaviour
    byte-identically.
  ...
  install-claude-code.sh has used since M032 P01 — required for
...
were added across M033/M037. Confirmed pre-existing on `main` via
