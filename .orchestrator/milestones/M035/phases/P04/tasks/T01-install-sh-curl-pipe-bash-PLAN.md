...
- `.orchestrator/DECISIONS.md` exists. Latest decision row is `### D008`
  (per `grep -nE '^### D[0-9]+' .orchestrator/DECISIONS.md` at
...
in `.orchestrator/DECISIONS.md` using the literal `### D### —` heading
...
4. **Append D009 to `.orchestrator/DECISIONS.md`.** Use Edit (not Write —
...
8. **Commit the work atomically (single commit per task).**
   ...
   ```bash
   git add packaging/install/install.sh tools/verify/m035-p04-install-sh-shape.sh .orchestrator/DECISIONS.md
   ...
   - .orchestrator/DECISIONS.md D009 — install.sh hosted at GitHub
...
- Artifact: `.orchestrator/DECISIONS.md` D009 row.
...
- `.orchestrator/DECISIONS.md` — agent reads only enough to confirm
  D008 is the latest decision row (`grep -nE '^### D[0-9]+' .orchestrator/DECISIONS.md
...
- `.orchestrator/DECISIONS.md` extends with the D009 block at the end
  of the file.
