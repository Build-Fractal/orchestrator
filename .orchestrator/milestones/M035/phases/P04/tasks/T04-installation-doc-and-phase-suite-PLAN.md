...
- `commands/update.md` exists with `## Update sources` H2 enumerating
  ...
  returns one match).
- `.orchestrator/DECISIONS.md` exists with D009 + D010 recorded.
...
- D011 row in `.orchestrator/DECISIONS.md` (release cadence: manual
...
5. **Append D011 to `.orchestrator/DECISIONS.md`.** Use Edit. Locate
...
11. **Commit atomically.**
    ...
    ```bash
    git add references/installation.md commands/update.md .orchestrator/DECISIONS.md tools/verify/m035-p04-installation-doc-curl.sh tools/verify/m035-p04-update-skill-doc-curl.sh tools/verify/m035-p04-phase-suite.sh
    ...
    - .orchestrator/DECISIONS.md D011 — manual stable releases pre-1.0
...
- Artifact: `.orchestrator/DECISIONS.md` D011 row.
...
- `.orchestrator/DECISIONS.md` — agent reads enough to confirm D010 is
...
- `.orchestrator/DECISIONS.md` D011 row appended.
...
context-switch to .orchestrator/DECISIONS.md to understand the
...
the M033 friendly-tester pass deadline (≤ 2026-05-12 per
