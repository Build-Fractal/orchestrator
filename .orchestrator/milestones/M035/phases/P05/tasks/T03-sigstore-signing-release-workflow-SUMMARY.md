...
- **Real OIDC issuance is not exercised by T03 verifier** — the verifier is grep-based static analysis of the workflow YAML. Real OIDC issuance (cosign sign-blob actually contacting Fulcio + Rekor under the GHA OIDC identity) only fires when the canonical repo (`Build-Fractal/orchestrator`) sees a v* tag-push event. T05 acceptance test exercises the cosign verify-blob path against a fixture install.sh + .sig + .pem + SHA256SUMS quartet under default-OFF live mode (M030 P03 live-LLM precedent — verifier passes via `SKIP:` for the cosign live-verify path unless the operator opts in via env var).
