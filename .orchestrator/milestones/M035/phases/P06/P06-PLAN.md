...
- **Implicit #Q-P06-1 (5-condition suppression matrix mapping for
  `update_run` JSONL emission)** → honor M027's 5-condition matrix
...
- `tests/m035-acceptance/run-acceptance-battery.sh` exists, executable,
  ...
  one of the SCs it's reporting against. Mirrors the M029/M030/M031/M032/M037
...
- `.orchestrator/DECISIONS.md` (modified — appends D012 update_source
...
- `.orchestrator/milestones/M035/M035-VALIDATED` (create, marker file
  with timestamp)
- `.orchestrator/milestones/M035/M035-SUMMARY.md` (create, populated per
...
- `.orchestrator/DECISIONS.md` (modify — D012 / D013 / D014)
...
- `.orchestrator/milestones/M035/M035-SUMMARY.md` (create)
...
- **Rule 1 (Prerequisite-existence)**: `scripts/state/read-config.sh`,
  ...
  `.orchestrator/config.yml`, `.orchestrator/DECISIONS.md`,
...
- **Rule 6 (Path-collision)**: `ls -la` performed against every
  ...
  - `.orchestrator/milestones/M035/M035-SUMMARY.md` — ABSENT
  No collisions; every new verifier and fixture path carries the
  `m035-p06-` slug per the milestone-prefix convention.

**M033 friendly-tester gate disposition:** the M033 friendly-tester
