...
- **`.orchestrator/DECISIONS.md`** exists. T01 appends D012 in the
...
1. **Schema registration** — `update_source` joins the canonical
   ...
   per the M027 P02/P03 multi-key co-location pattern. The append is a
   single-line edit; it does not change the read-config.sh CLI shape.

2. **D012 decision row** — appended to `.orchestrator/DECISIONS.md`
...
2. **Append D012 to `.orchestrator/DECISIONS.md`**. Read the file at
   execution time to determine the prevailing row format (table-row
   shape as used by D001..D006 / D-RN-1..D-RN-7, OR the literal
...
3. **Author the verifier**
   ...
   4. `.orchestrator/DECISIONS.md` contains the literal token `D012`
      (case-sensitive grep -F).
   5. `.orchestrator/DECISIONS.md` contains the literal token
...
- `.orchestrator/DECISIONS.md` modified — contains a `D012` row
...
- `.orchestrator/DECISIONS.md` — T01 appends D012; row format mirrors
