...
- `.orchestrator/DECISIONS.md` exists (verified at plan-authoring time, 33 lines containing 28 row entries — see Notes for the actual scope clarification).

## Description

Restructures `.orchestrator/DECISIONS.md` from a 7-column markdown table to `### Title { #dr-code-nnn }` heading entries with a code-chip body markup, preserving every legacy `#dr-code-nnn` permalink anchor via `attr_list`. Authors a framework-owned `scripts/verify/decisions-shape-lint.sh` that enforces the new shape (per #Q-7 resolution). Authors `tests/m037-acceptance/p01-dr-heading-shape.sh` invoking the lint plus a permalink-resolution check across cross-referencing files in the repo.
...
1. **Read `.orchestrator/DECISIONS.md` in full**. The current shape is a markdown table:
...
4. **Author `scripts/verify/decisions-shape-lint.sh`** (framework-owned per #Q-7 resolution; ships in install bundle). Single-script-file shape per AD-19. The script:
   - Accepts a single argument: path to a DECISIONS.md file (defaults to `.orchestrator/DECISIONS.md` in CWD).
...
6. **Author `tests/m037-acceptance/p01-dr-heading-shape.sh`** (acceptance test, SC-3). The test:
   - Invokes `bash scripts/verify/decisions-shape-lint.sh .orchestrator/DECISIONS.md` and asserts exit 0.
...
- Expected output of `scripts/verify/decisions-shape-lint.sh`: `PASS: decisions-shape-lint .orchestrator/DECISIONS.md (28 entries, all anchors unique)`.
...
- `.orchestrator/DECISIONS.md` — restructured in place. 28 row entries become 28 `### Title { #dr-code-nnn }` heading blocks.
...
- `.orchestrator/DECISIONS.md` restructured: zero rows match `^| D[0-9]+ |`; every `### ` heading matches `^### .+ \{ #(dr|bg|an|mem|q)-[a-z0-9-]+ \}$`; every legacy `#dr-code-nnn` permalink target preserved.
