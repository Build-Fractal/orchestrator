...
3. **Apply identical changes to `packaging/install/install-codex.sh` and `packaging/install/install-cursor.sh`** for parity. Find the equivalent `cfg_target` write block in each (the three installers share the same M032 configuration block shape per the M032 dogfood findings) and apply the same replacement.
