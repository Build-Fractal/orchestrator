...
# Dispatch Context -- T04-wiki-deploy-url-and-suite (Phase P01, Milestone M043)
## Manifest
...
| Upstream Context | 981-1013 | ~600 | required |
| Task Plan | 1015-1175 | ~2000 | required |
| State Context | 1177-1183 | ~100 | required |
| First-Turn Completeness | 1185-1220 | ~400 | required |
| **Total** | | **~14500** | |
...
1. **Branch the URL-print tail in `scripts/wiki/wiki-deploy.sh`.** Use the Edit tool. Insert the cloudflare branch BEFORE the existing github-pages print block, leaving that block verbatim:
   ...
   - `new_string`:
     ...
     M043_DEPLOY_TARGET="$(bash "$REPO_ROOT/scripts/wiki/resolve-deploy-target.sh" "$ROOT" 2>/dev/null || echo github-pages)"
...
- **Milestone**: M043
- **Phase**: P01
- **Task**: T04-wiki-deploy-url-and-suite
- **Tier**: C
