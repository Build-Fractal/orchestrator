...
- [x] **P02**: Producer/consumer format unification + round-trip oracle (BUG-A) — "A decision appended by `append-decision.sh` on a Quick fixture, rebuilt, then resolved by `filter_decisions`, byte-equals the captured scope/choice fields (observed awk `$5`/`$6` land on Scope/When); a flat `## K###` entry passes `kf_filter_stream` and appears in the inject."
  - Risk: high
  - Depends: P01
  - Boundary Map:
