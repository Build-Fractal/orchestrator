...
4. **Wiki tooling reconciliation** — M032 just shipped (2026-05-05) with
   `wiki-init.sh --deploy` baking in-tree `wiki/` assumptions. M037 just
...
5. **M035 packaging assumptions** — P00–P05 closed (or are about to close)
...
7. **M033 verifiers (`tools/verify/m033-*.sh`)** — many grep for in-tree
...
- **Related deferred work**:
  `.orchestrator/proposals/post-launch-wiki-ux-and-adapters.md` — also
...
- **Constitution**: Principle XVI (Distribution Surface Integrity) — if
  amended per `.orchestrator/proposals/constitution-amendment-inclusion-criteria.md`,
...
- **M035-packaging-distribution** —
  `.orchestrator/proposals/M035-packaging-distribution.md`. P06
