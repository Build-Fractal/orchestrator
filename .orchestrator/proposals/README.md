...
| `Mxx-auto-v2-claude-code-loop-integration.md` | Auto v2 — one `orchestrator:auto` entry that classifies tier (A/B/C), sizes the loop shape, and selects a Claude-Code-native loop *substrate* (self-continuing attended via `ScheduleWakeup`/self-paced `/loop`; unattended via headless `claude -p` driver or cloud routine; unit-grain until-verified via `Stop` hook) — all driving today's `auto-loop.sh` state machine unchanged. Closes three weaknesses of the current loop: needs babysitting across context-rotation, attended-only, and only-Tier-C-gets-a-loop. Captured 2026-06-30; `discuss` outcomes locked (see proposal §Discussion Outcomes): **split into `M-auto-v2a`** (Posture 1 self-continue only, standalone first) **+ `M-auto-v2b`** (unified entry + `do` merge + Postures 2/3 + runtime degrade). Unattended = both substrates (headless primary), explicit per-run `--unattended` opt-in only. | Two milestones (v2a small/standalone; v2b ~4–5 phases) | Post-launch power-user band (M034/M038/M040 tier); hard-dep M028 hook portability; companion to `orchestrator-exec-primitive` + M009 (runtime degradation) |
...
| `corpus-exhaustion-gate.md` | Mandatory, evidence-producing **corpus-exhaustion gate** before any question reaches an operator/SME and before any plan/spec/roadmap finalizes: extract search terms → grep-sweep a configurable store manifest → batched conversus judge (`ANSWERED`/`PARTIAL`/`IRREDUCIBLE`) → drop answered, surface only irreducible → emit per-question audit artifact; no artifact → BLOCK (reuses `BG-###` / conversus-gate / verify-ladder convention). Hardens the soft "exhaust corpus first" habit into structure. Captured 2026-05-30 from PBJ Analyzer — operator estimate ~9/10 drafted questions are already answered in stored knowledge. **Strong overlap with M040's decision-contradiction gate** (shared two-agent-conversus-sweep + PASS\|BLOCK + human-gated-apply plumbing). | Phase 1 single PR (blocking artifact checklist) + Phases 2–3 milestone-shaped (`corpus-gate` skill + doctor lint/telemetry); provisional ID | Phase 1 yes (standalone paper-cut, no demand-signal). Phases 2–3 demand-driven post-launch — evaluate absorption into M040 at queue-entry, slot in the M034/M038/M040 decision-packet family |
| `M043-wiki-cloudflare-access-deploy-target.md` | Cloudflare Pages + Access as a first-class (recommended-for-private) wiki-deploy target: `wiki.deploy_target: github-pages \| cloudflare-access` switch + `wiki-cloudflare-deploy.yml` template (`npx wrangler`, NOT `wrangler-action`) + idempotent `cloudflare-access-setup.sh` (Pages project + Access app/policy, apex+wildcard, before-deploy ordering) + installation.md pitfall docs + `status`/`doctor` warning on the (private repo + github-pages + non-Enterprise) tuple. Fixes the GitHub-Pages footgun: private Pages is Enterprise-Cloud-only, so on Free/Pro/Team a private repo either silently goes public or silently 422s while the build stays green. | Milestone (3 phases + optional P04 friendly-tester/live-deploy pass); provisional ID M043 | Post-launch fast-follow — **demand-driven by a live incident** (pbj-central, validated 2026-06-04). Sequence after M041 (`scripts/wiki/` carve-out) so the new setup script auto-distributes |
