...
| `dynamic-workflows-integration.md` | Claude Code dynamic-workflows audit + capability-gating contract. Strong fit for Principles V/II/I-payload, hard conflict with VI (script-variable state lost on CC exit). Adopt-narrowly: inner/outer split — `orchestrator:auto` stays disk-durable milestone authority, workflows only as within-session disk-checkpointed inner fan-out. §4 (the load-bearing piece): gate on *capability* (`parallel_subagent_fanout`) not runtime identity, extend `detect-capabilities.sh` + `RUNTIME-ASSUMPTIONS.md` registry (seeds M009) + forced-fallback CI lane vs Principle-VIII rot. §5 pilot: parallel reference-corpus extraction (`extract.md`). §6: detective sweep / M023 design personalities / consolidate fan-out. | Mixed: 1 standalone gating PR (**§4 SHIPPED 2026-05-29**) + 1 demand-driven post-launch pilot (§5) + 3 deferred opportunities (§6) | Partly done — §4 shipped (capability probe + registry + forced-fallback test, 9/9 green); §5/§6 demand-driven post-launch. Captured 2026-05-29 (audit ran as a dogfood 14-agent workflow) |
