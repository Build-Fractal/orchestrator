...
| `orchestrator:start` (M033) | NO | Owns onboarding flow; cannot be subagent-invoked |
...
| `orchestrator:do` (M031) | NO | Universal entry — invokes other commands; nesting risk |
...
1. **YAML or JSONL for the data file?** YAML is more human-readable for a small static table; JSONL composes better with the existing M019 emitter pattern. Recommendation: YAML, since this is configuration not event stream.
