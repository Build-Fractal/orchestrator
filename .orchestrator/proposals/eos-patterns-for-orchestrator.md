...
| **Scorecard** | `scripts/observability/metrics-rollup.sh`, M027 efficiency-footer, M019 Tier 1 JSONL | Exists, but not framed as a one-owner-per-row weekly leading-indicator board. |
...
| **People Analyzer / GWC** | (Implicit in M030 model selection) | No explicit fitness check per seat × skill/model pairing. |
...
The single most load-bearing EOS pattern is **one document, all functions, two pages, 10-year on the left, 90-day on the right**. The orchestrator already has the components scattered across `CLAUDE.md`, `milestone-summary.md`, and per-milestone files. Extend the queued M029 `orchestrator:where` to render a V/TO-shaped page:
...
These belong in the constitution (or a new "operational invariants" doc) as hard, enforceable limits. The orchestrator already has hooks (M021/M028 shape-guards) — these are a natural place for cap enforcement.
...
- **Rigidity.** EOS's "do all six components or it doesn't work" stance is a known failure mode for creative/research-shaped orgs. Orchestrator-side: every adopted pattern should be **opt-in and reversible**, the same posture we took for M013 GitHub integration (FR-11).
...
6. **Constitution amendment** (independent PR, any time): Principle VIII + the hard caps as operational invariants. Bundles with the standalone constitution amendment already queued (`.orchestrator/proposals/constitution-amendment-inclusion-criteria.md`).

The launch sequence (M032+M033 paired → M029 → M035 P02–P06) is not disturbed — items 2 and 3 fold into existing milestones; items 4 and 5 are post-launch fast-follows; the constitution amendment is independent.
