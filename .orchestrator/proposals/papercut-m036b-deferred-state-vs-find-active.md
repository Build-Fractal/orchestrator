...
**Captured**: 2026-05-05 (during M029 entry readiness check)
**Shape**: Single PR, ≤1 day
**Independent**: Yes — no milestone needed.

## Problem

`scripts/state/find-active-milestone.sh` returns `M036 planning C` because M036's roadmap has P08 + P09 (the M036b post-launch slice) marked `[ ]` unchecked, and `derive-phase.sh` Rule 3b ("Active phase has no plan file → planning") fires on the first unchecked phase.
...
- Future M036b work opens as a new milestone (probably M037 by then, or M036b/ as a sibling directory if the convention is honored).
