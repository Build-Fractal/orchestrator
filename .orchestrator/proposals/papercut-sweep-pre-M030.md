# Paper-Cut Sweep PR Brief — Pre-M030
...
- M032 invariant encoding for "project-owned paths must not collide with staged dirs" — text-only amendment deferred until M032 enters planning per CLAUDE.md. Item #19 (planner template `tools/verify/`) still ships in this PR; only the spec-side invariant defers.
...
### Group 7 — Stale verifiers (M025/M013 contract drift)
...
- `m013-p04-post-verify-hook.sh` was already failing pre-M028 (asserts `hook_count=6` and `event: "post_verify"` JSON keys that the M025 baseline emission has never produced — its schema is `hooks.{Stop,PreToolUse}`-keyed).
...
System-prompt-staged guidance recommends `git commit -m "$(cat <<'EOF' ... EOF)"` for multi-line messages, but the active M021 PreToolUse Bash shape-guard rejects this shape under AP-008.
...
1. Read this brief (`.orchestrator/proposals/papercut-sweep-pre-M030.md`).
