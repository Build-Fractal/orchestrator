...
**Why this preserves M012's pattern**: stubs stay committed (no auto-commit, no derived-artifact drift between local serve and deploy). The diagnostic *catches* drift loudly rather than papering over it.
...
**Folds into**: M035 P00 or P01 — composes naturally with the `--mode=symlink` install + version-drift warning pre-launch dev-ergonomics scope. Layer 1 is **strictly pre-launch**, not deferrable: PBJ-central's daily wiki cadence is the exact dogfood loop M037 was supposed to enable, and this paper-cut directly contaminates that loop.
