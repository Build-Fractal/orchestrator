...
Validate that both branches of `scripts/dispatch/build-context.sh` correctly integrate with the M002 knowledge architecture. The existing code already has knowledge index support (added during M001/M005 refactoring), but P04 needs to verify it works end-to-end with the P01-P03 delivered scripts and fix any integration issues.
