...
Update `check_existing_state` so it correctly detects pre-existing orchestrator state under the new `$target_root` semantics established by T01, and so it covers BOTH M008 canonical roots:
