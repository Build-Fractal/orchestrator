...
The checkpoint lives at `.specify/orchestrator/milestones/<milestone>/engine-checkpoint.json` (sibling of `execution-log.jsonl`, `summary.md`, etc.). Writes are atomic via temp-file-then-`mv` (inherited M002 convention — see `scripts/lifecycle/lock-manager.sh` for reference). The JSON payload is assembled with `printf` (no `jq`); reads are grep/sed-based.
