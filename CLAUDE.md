...
## Forward Roadmap (revised 2026-05-01 — post-M030 close)

M028 closed 2026-04-29. M030 (adaptive model selection) closed 2026-05-01: 8 phases (P00–P07), 14 success criteria verified via the M030 acceptance battery (`tests/m030-acceptance/run-acceptance-battery.sh` → `BATTERY: pass=22 fail=0`); `M030-VALIDATED` marker + `M030-SUMMARY.md` + milestone-grain `unit_close` all on disk; `validate-milestone.sh` reports 197/197 PASS. Remaining pre-launch queue:

**M031 → M032 → M033 → M029 → M035 → launch**

**Parallel pre-launch workstream**: **M036a (reference-corpus ingest pre-launch slice, P00–P07)** — PBJ-pilot-deadline-driven (2026-05-15). Orchestrator-owned tiered extraction (Tier 0 manifest + Tier 1 deterministic adapters + Tier 2 LLM via M030 + conversus fidelity gate) + ingest + dispatch injection. Inserts ahead of M035 P02–P06 if the orchestrator launch event slips past mid-May. Spec amended 2026-05-01 (extraction-ownership flip + tier model + M036a/M036b split).
...
- **M036 (reference-corpus ingest, split 2026-05-01 into M036a pre-launch + M036b post-launch)** — extends the knowledge layer with a `reference/` chunk family for non-spec materials (regulatory, training, glossary), three new edge types (`cites` / `derived_from` / `applies_to_field`), `[source:...]` tag namespace, dispatch injection under a token-budget governor, and **orchestrator-owned tiered extraction** (Tier 0 manifest + binary preservation, Tier 1 deterministic shell adapters for PDF/DOCX/XLSX/MD, Tier 2 LLM-driven structured Markdown via M030 routing under conversus fidelity gate). Spec authored 2026-04-30, **amended 2026-05-01** when PBJ Analyzer clarified it has no path-B extractor — extraction-ownership flip required (NG-3/4/5 inverted/narrowed); tier model added as core architecture; DOCX added; #Q-3 (versioning → supersede chain) and #Q-5 (fidelity gate → tiered) resolved at amendment. Roadmap restructured 2026-05-01 from 7 phases → 10 phases under M036a/M036b split.
  - **M036a (pre-launch urgent, P00–P07, 8 phases)** — PBJ-pilot-deadline-driven (2026-05-15). Critical path: P00 foundation → P01 Tier 1 adapters → P02 Tier 0 + extract command → P03 Tier 2 + conversus gate → P04 ingest → P05 graph → P07 dispatch injection (P06 idempotency required for production but not first-run pilot). May insert ahead of M035 P02–P06 if orchestrator launch event slips.
