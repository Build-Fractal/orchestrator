...
| Inherited — Tier 2 XXII (Distribution Surface Integrity) | XXII (Distribution Surface Integrity) | **Provisional** (criterion (i) PASS 2026-05-11; verifier-stub maturity per the PENDING/ACTIVE cap below) | Tier 2 inheritance ratified 2026-05-11 by the constitution v2.2.0 amendment ([`proposal`](./.orchestrator/proposals/constitution-amendment-inclusion-criteria.md); decision packet at `.orchestrator/comments/review-queue/2026-05-11-XXII-XII-blind-substantive-findings.md`). **Canonical version source**: `CHANGELOG.md` top-line `## [X.Y.Z]` heading (consumed by `packaging/install/install-{claude-code,cursor,codex}.sh` per `install-claude-code.sh:524`). **Evidence**: stub scripts at `scripts/verify/version-source-of-truth.sh`, `scripts/verify/manifest-coverage.sh`, `scripts/verify/installer-smoke.sh` (path-existence discharges Criterion 1 feasibility; PENDING/ACTIVE maturity tier with comprehensive promotion-conditions enumeration declared in [§ Tier 2 XXII — PENDING/ACTIVE verifier-stub cap](#tier-2-xxii--pendingactive-verifier-stub-cap) below — Item 6 of `.orchestrator/proposals/M0XX-tier-2-xxii-xii-substantive-followups.md`). |
...
| Installer-script config-knob shapes inside `packaging/install/install-*.sh` (e.g., `RUNTIME=`, `BUNDLE_ROOT=`, channel-specific version strings) | Behavioral correctness of the end-to-end installer surface — verified by Invariants 1–3 (single-source versioning, force-include discipline, end-to-end install smoke) | Inherited conversus Tier 2 XXII (Distribution Surface Integrity) | Explicitly **NOT** governed by `check-dead-infra.sh` — installer-mechanics variables would produce false positives if linted as config knobs. Evidence stubs at `scripts/verify/version-source-of-truth.sh`, `scripts/verify/manifest-coverage.sh`, `scripts/verify/installer-smoke.sh` (PENDING/ACTIVE per Item 6 cap entry below). |
...
| `scripts/verify/installer-smoke.sh` | PENDING | Present (32 lines; emits TODO line; `exit 0` regardless of state) | (1) Invoke each `packaging/install/install-*.sh` against a fresh project fixture (per Decision A's pending "fresh project fixture" operational definition; the rerun proposes language under Item 3 that is deferred until Decision A lands); verify `orchestrator:status` exits 0 with the structured status format; emit `verdict=PASS\|FAIL`. (2) Green-path fixture: a clean tmpdir where the installer + `orchestrator:status` both succeed. (3) Red-path fixture: a deliberately-broken installer invocation (e.g., a corrupted bundle manifest) where the smoke harness flags the failure. (4) CI wiring (release-tag automation is the natural fire point). (5) Promotion entry. |
