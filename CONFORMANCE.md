...
|---|---|---|---|
| `scripts/verify/version-source-of-truth.sh` | PENDING | Present (28 lines; emits TODO line; `exit 0` regardless of state) | (1) grep `packaging/install/install-*.sh` and `packaging/bundle/*/manifest.txt` for hardcoded version strings; emit `verdict=PASS\|FAIL` and offending lines on FAIL. (2) Green-path fixture: a state where `CHANGELOG.md` top-line `## [X.Y.Z]` heading is the only version source. (3) Red-path fixture: a state where an installer script embeds a hardcoded version literal. (4) CI wiring. (5) Promotion entry. |
