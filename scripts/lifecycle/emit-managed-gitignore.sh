...
if [ -n "$BLOCK_CONTENT_FILE" ]; then
  ...
  # Canonical default body (FR-6 v1).
  {
    ...
    printf '\n'
    printf '%s\n' "# --- M035 rollback sidecars (regenerated by write-rollback-marker.sh) ---"
    printf '%s\n' "# Per-install snapshot of the prior version's manifest + a marker file"
    printf '%s\n' "# tracking prior_version / prior_commit_sha / prior_manifest_path /"
    printf '%s\n' "# prior_install_mode / rolled_at. Consumed by install-claude-code.sh"
    printf '%s\n' "# --rollback. Local state — must not be committed cross-machine."
    printf '%s\n' ".orchestrator/.previous-version"
    printf '%s\n' ".orchestrator/.rollback/"
    printf '\n'
    printf '%s\n' "# --- Wiki readability decorator output (regenerated by wiki-decorate-build.py) ---"
    printf '%s\n' "# Decorated copies of .orchestrator/**.md emitted by"
    printf '%s\n' "# scripts/wiki/wiki-decorate-build.py. Wiki stubs include from .staged/"
    printf '%s\n' "# (rewrite-relative-urls=false) so codes/§-refs/paths render as live links"
    printf '%s\n' "# with hover-tooltip titles. Regenerated from .orchestrator/ source on each"
    printf '%s\n' "# decorate-build run; nothing durable lives here."
    printf '%s\n' "wiki/.staged/"
  } > "$BODY_TMP"
fi
