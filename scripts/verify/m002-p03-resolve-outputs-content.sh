#!/usr/bin/env bash
# Verifies resolve-entries.sh outputs the body content of resolved entries.
set -eu
TMPDIR="$(mktemp -d)"
...
# --- Setup ---
create_entry "MEM001" "convention" "[]" "First entry for resolve"
create_entry "MEM002" "gotcha" "[]" "Second entry for resolve"
...
if ! echo "$output" | grep -q "Body text for MEM001"; then
  echo "FAIL: resolve-entries.sh output missing body text for MEM001"
  exit 1
fi
if ! echo "$output" | grep -q "Body text for MEM002"; then
  echo "FAIL: resolve-entries.sh output missing body text for MEM002"
  exit 1
fi

echo "PASS: resolve-entries.sh outputs body content of both MEM001 and MEM002"
