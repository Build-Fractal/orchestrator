#!/usr/bin/env bash
# scripts/verify/m018-p02-dual-write-recent.sh — phase-truth verifier:
# "CLAUDE.md and AGENTS.md `recent-changes` blocks both name M018/P02".
#
# Both files contain a delimited block:
#   # >>> orchestrator:recent-changes >>>
#   ...
#   # <<< orchestrator:recent-changes <<<
# The block in each file MUST contain the literal substring `M018/P02`.
...
check_block() {
  ...
  if grep -qF 'M018/P02' "$block_file"; then
    printf 'PASS: M018/P02 named in %s recent-changes\n' "$label"
    rm -f "$block_file"
    return 0
  fi

  printf 'FAIL: M018/P02 not named in %s recent-changes block\n' "$label" >&2
  rm -f "$block_file"
  return 1
}
