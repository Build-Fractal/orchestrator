#!/usr/bin/env bash
# scripts/verify/m018-p03-dual-write-recent.sh — phase-truth verifier:
# "CLAUDE.md and AGENTS.md `recent-changes` blocks both name M018/P03
# (or tier1) — phase-close dual-write via scripts/util/dual-write-runtime-md.sh."
#
# Two assertions:
#   1. Both CLAUDE.md and AGENTS.md exist at repo root.
#   2. Both files' marker-bounded `# >>> orchestrator:recent-changes >>>`
#      regions contain a line matching `M018/P03` OR `tier1` (literal
...
for f in "$REPO_ROOT/CLAUDE.md" "$REPO_ROOT/AGENTS.md"; do
  ...
  if ! printf '%s\n' "$block" | grep -qE 'M018/P03|tier1'; then
    printf 'FAIL: %s recent-changes block missing M018/P03 or tier1 marker\n' "$f" >&2
...
printf 'PASS: m018-p03-dual-write-recent (both CLAUDE.md and AGENTS.md name M018/P03 or tier1 in recent-changes)\n'
exit 0
