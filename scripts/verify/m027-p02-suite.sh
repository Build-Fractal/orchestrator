...
#
# Runs all 11 per-contract m027-p02-*.sh verifiers in stable order
# (cheapest static checks first; latency / live-invocation last) and
# aggregates results.
#
#   - Exit 0 on all-green; prints `PASS: m027-p02-suite.sh <N> gates`
#     to stdout.
#   - Exit 1 on any failure; prints `FAIL: m027-p02-suite.sh <N> failing
...
#     output. Mirrors the shape of m027-p01-suite.sh / m027-rollup-schema.sh.
#
...
# m027-p02 literal token below also satisfies the must-have content
# check.
GATES="
...
echo "SUMMARY: m027-p02-suite.sh pass=$pass_count fail=$fail_count total=$total_count"

if [ "$fail_count" -eq 0 ]; then
  echo "PASS: m027-p02-suite.sh $total_count gates"
  exit 0
else
  trimmed_failed="$(echo $failed_gates)"
  echo "FAIL: m027-p02-suite.sh $fail_count failing gates: $trimmed_failed" >&2
  exit 1
fi
