...
- **FR-2 (query-surface)**: A dispatch-callable query surface (`scripts/knowledge/query.sh`, with a one-line wrapper in `scripts/dispatch/dispatch-interface.sh`) resolves `topic=<X> [state=<S>]` queries with the following deterministic semantics: (a) **exact-match** is case-insensitive whole-string equality against the frontmatter `topic:` field (no tokenization, no stemming, no partial matches); (b) the **topic-keyword-index** is the case-folded set of frontmatter `tags[]` values across all entries, rebuilt lazily on each query (per context-draft AD-2); (c) **matching**: an entry matches if its `topic:` field equals `<X>` (case-insensitive) OR `<X>` (case-folded) appears in its `tags[]` list; (d) **state filter**: when `state=<S>` is supplied, only entries with that exact `status:` are returned; default state filter when unspecified is `graduated` only; (e) **ranking**: `topic:`-field exact matches rank above tag-only matches; ties within a tier are broken by `last_verified` descending; (f) **output**: `--format ids` (default) emits one `entry_id=<ID>` line per match in rank order; `--format json` emits a single JSON document `{"matches": [{"id": "<ID>", "title": "<title>", "status": "<state>", "rank": <N>}, ...]}`. Returns metadata only — never streams entry bodies (CON-2). Semantic/hybrid resolution is deferred unless M024 routing demonstrates real need (CON-3). Satisfies US-1.
...
- **FR-5 (jaccard-clustering)**: `orchestrator:consolidate --cluster` computes pairwise Jaccard similarity on the feature vector defined in CON-5 and proposes clusters at or above a configurable threshold; emits cluster IDs consumable by `graduate.sh`. Satisfies US-4.
...
- **SC-1**: `bash scripts/knowledge/query.sh --topic <X>` against a fixture with three graduated + two candidate entries on topic X (matching by both `topic:` field and `tags[]` membership) returns exactly the three graduated entries; default `--format ids` emits stdout matching `^entry_id=<ID>$` lines only for graduated entries; ranking places `topic:`-field exact matches before tag-only matches; exit 0. `--format json` against the same fixture emits a single JSON document parseable by `jq`, with a `matches` array of length 3 in rank order. (US-1, FR-2, FR-8)
