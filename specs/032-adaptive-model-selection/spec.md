...
As an orchestrator operator running real milestone dispatches, I want the classifier to select a model and the routing-table choice recorded in JSONL alongside the actually-used model, so I have empirical paired data validating that "lesser model X performs equivalently on task class Y" before any live cost change.

**Why this priority**: Constitution Principle II (Evidence Before Claims) forbids flipping live routing on intuition. The shadow record is the evidence. Without 50+ paired dispatches, US-3 has nothing to defend its flip on.

**Independent Test**: Run 50 real dispatches across at least two phases of any milestone with `M030_SHADOW_MODE=1`. Each `dispatch_usage` JSONL record contains both `model_routed` and `model_used` fields; `model_used` continues to match the runtime default; `metrics-rollup.sh --shadow-comparison` reports per-class verifier pass rates and outputs a recommendation table.
...
2. **Given** 50+ shadow dispatches on disk, **When** the shadow-comparison script runs, **Then** stdout reports per-class verifier pass rate for the routed model versus the actual model and surfaces any class with >10% degradation as a `block_flip` recommendation.
3. **Given** the shadow comparison shows no class has >10% degradation, **When** the operator runs the flip-readiness check, **Then** stdout reports `ready_to_flip=true` and lists the per-class evidence count.
...
- **FR-7 (shadow-mode)**: When `M030_SHADOW_MODE=1` (env var) or `model_routing.shadow: true` (config), `dispatch-interface.sh` runs the classifier and records the routing decision but passes the runtime default to the adapter. JSONL `dispatch_usage` records gain both `model_routed` (symbolic + ID) and `model_used` (actual ID). Shadow mode is the default state until US-3 flip-readiness is signed off. Satisfies US-2 AS-1, US-2 AS-2.
- **FR-8 (shadow-comparison)**: `scripts/diagnostics/shadow-compare.sh` aggregates the shadow corpus and reports per-class verifier pass rates, mean cost delta, and a `flip_recommendation=ready|block` verdict. Threshold for `ready`: ≥50 dispatches per class with verifier-pass-rate degradation ≤10% relative to the actual-model baseline. Satisfies US-2 AS-2/AS-3 and the brief's open-question resolution #3.
- **FR-9 (live-routing-flip)**: When `model_routing.live: true` (config) and shadow corpus passes the flip-readiness check, `dispatch-interface.sh` passes the routed model ID to the adapter (i.e., shadow mode disengages). The flip is per-project and reversible (revert the config knob); orchestrator state and prior shadow data are preserved across the flip. Satisfies US-3.
...
- **FR-12 (per-milestone-floor)**: `.orchestrator/config.yml` `model_routing.min_tier: <symbolic-tier>` raises the effective floor for every dispatch in the active milestone — even `mechanical`-classified tasks dispatch at the floor or higher. JSONL records `override_source=milestone_floor`. Satisfies US-4 AS-2.
...
- **SC-2**: After 50+ shadow dispatches across at least two phases, `bash scripts/diagnostics/shadow-compare.sh` exits 0 and stdout contains per-class evidence counts ≥50 plus a `flip_recommendation=` line with one of `ready` or `block`.
...
- **SC-8**: `bash scripts/diagnostics/metrics-rollup.sh --by-model` exits 0 and stdout contains a line matching the regex `^[0-9]+ dispatches: [0-9]+ fast / [0-9]+ balanced / [0-9]+ smart`.
...
- **SC-10**: A fixture corpus of ≥30 hand-labeled tasks classifies with ≥85% agreement against ground truth — measured by `bash scripts/dispatch/classify-task.sh` output matching the human label for ≥26 of 30 plans.
...
- **CON-4 (kill-switch-always-available)**: The kill switch (`model_routing_enabled: false`) MUST disable the entire routing layer regardless of any other config. No code path may bypass it. This is the operator's panic button if routing introduces an unexpected regression.
...
- **Principle II (Evidence Before Claims)**: The shadow-mode validation (FR-7 / FR-8 / SC-2) is the embodiment of this principle. No live-routing flip happens without 50+ paired dispatches showing per-class verifier-pass-rate degradation ≤10%. The brief's "intuition is not enough" rationale aligns directly.
...
- **#Q-1 (classifier-fixture-source)**: For the ≥30-task ground-truth corpus (SC-10), should we hand-label PLAN.md files from this repo's own milestone history, synthesize fixture plans, or both? Plan-phase decides; recommendation is to draw from real milestone history (specs/0NN-*) so the heuristics are tuned against actual orchestrator workload, not synthetic edge cases. **Note**: Conversus arbiter ruled this question must be promoted to a spec constraint, not deferred — see Q-A4 / RISK-05 below.
...
### Conversus Gate Findings (verdict=BLOCK, advisory — deferred to discuss)

The Standard-intensity adversarial gate (`conversus.sh gate spec-pressure-test`) returned **verdict=BLOCK, surviving_disputes=7** with one arbiter ruling. Full deliberation at `specs/032-adaptive-model-selection/conversus/`. At Standard intensity the BLOCK is advisory — these findings carry into `orchestrator:discuss` for resolution before plan-phase. Two are CRITICAL (architecturally load-bearing) and must be resolved before planning begins; the rest are targeted prose/logic amendments.
