...
- **CON-1 (bounded-not-fresh re-entry)**: The `ScheduleWakeup` / self-paced `/loop` primitive re-fires within the same session; it is NOT a new OS process. Context relief across the boundary therefore comes from the harness's own context management (summarization/compaction at window pressure), not from a process reset. This is acceptable ONLY because state-on-disk is authoritative (CON-2). The truly-process-fresh path is deferred to the v2b headless driver. (See #Q-1.)
