#!/usr/bin/env bash
# tools/verify/m031-p02-scope-guard.sh -- M031 P02 SC-12 scope-guard verifier.
#
# Asserts the P02 diff (working tree vs HEAD) does NOT touch any path
...
#
# SC-12 forbidden paths (block-list -- inherited verbatim from P01):
...
# orchestrator-emitted hit_count drift on knowledge/MEM*.md is a
# dispatch side-effect, not a manual P02 scope violation. When a
# `knowledge/(conventions|lessons|patterns)/MEM*.md` file shows up in
# the diff, the verifier inspects its actual diff content; if every
# changed line matches `^[+-]hit_count: [0-9]+$` the path is carved
...
#
# Allow-list (P02 "Files Likely Touched" per P02-PLAN.md): only the
# files listed below + the permissive `.orchestrator/observability/`
# and `.orchestrator/tier-a-plus/` prefixes may legitimately differ
# from the M031 baseline. The `.orchestrator/tier-a-plus/` prefix is
# the P02 equivalent of P01's `.orchestrator/observability/` carve-
# out -- per-flow scratch artifacts written during T04 router test
# runs.
#
...
# (`tools/verify/m031-p02-scope-guard.sh`) is in the allow-list, so
...
#
# Final line: SUMMARY: m031-p02-scope-guard.sh pass=N fail=M block_list_violations=K mem_hitcount_carveouts=L
...
ALLOW_LIST_EXACT='scripts/intake/shape-detect.sh
...
.orchestrator/milestones/M031/phases/P02/tasks/T01-classifier-and-provenance-SUMMARY.md
.orchestrator/milestones/M031/phases/P02/tasks/T02-slug-and-role-templates-SUMMARY.md
.orchestrator/milestones/M031/phases/P02/tasks/T03-prompt-and-prompt-ux-test-SUMMARY.md
.orchestrator/milestones/M031/phases/P02/tasks/T04-router-and-flow-test-SUMMARY.md
.orchestrator/milestones/M031/phases/P02/tasks/T05-phase-suite-and-scope-guard-SUMMARY.md
.orchestrator/milestones/M031/phases/P02/P02-SUMMARY.md'

# Allow-list prefixes (permissive; for JSONL emission records and
# per-flow Tier A+ scratch artifacts).
...
# Inherited verbatim from M031/P01/T04 remediation.
...
changed_paths_file="$( mktemp -t m031-p02-scope-guard.XXXXXX )"
...
printf 'SUMMARY: m031-p02-scope-guard.sh pass=%d fail=%d block_list_violations=%d mem_hitcount_carveouts=%d\n' \
