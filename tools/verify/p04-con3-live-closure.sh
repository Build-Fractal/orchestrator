#!/usr/bin/env bash
# tools/verify/p04-con3-live-closure.sh -- M030 P04 CON-3 closure gate.
#
# Asserts that the T02 live-routing amendment to scripts/dispatch/
# dispatch-interface.sh introduces zero NEW provider-model-ID literals.
# Compares per-pattern occurrence counts in:
...
#
# Mirrors the P03/T02 p03-con3-closure.sh shape.
#
...
if [ ! -f "$TARGET" ]; then
  ...
  printf 'SUMMARY: p04-con3-live-closure.sh pass=0 fail=1\n'
  exit 1
fi

HEAD_TMP="/tmp/p04-con3-head.txt"
...
_check_pattern() {
  ...
  local pattern="$1"
  local label="$2"
  local head_count_tmp="/tmp/p04-con3-headc.txt"
  local wt_count_tmp="/tmp/p04-con3-wtc.txt"
  ...
  if [ "$wt_count" -le "$head_count" ]; then
    pass=$((pass + 1))
    printf 'PASS: CON-3 closure pattern %s (%s) HEAD=%d, working-tree=%d\n' \
...
printf 'SUMMARY: p04-con3-live-closure.sh pass=%d fail=%d\n' "$pass" "$fail"
