#!/usr/bin/env bash
# tools/verify/p04-con5-no-fourth-record.sh -- M030/P04 CON-5 hard-cap.
#
# Asserts: with stub-fail-n.sh adapter and counter=4 (4 failures pending),
# the dispatch-interface escalation loop fires exactly 3 adapter
# invocations (initial + 2 escalations), then stops at the cap. The 4th
# would-be invocation NEVER happens.
#
# Same six assertions as SC-5 (cap behavior is identical) plus:
#   7. STUB_FAIL_COUNTER_INVOCATIONS_FILE contains exactly 3 lines
#      (proving the stub adapter was called 3 times, not 4).
#
# This is the load-bearing CON-5 gate: the hard-cap is enforced at the
# adapter-invocation site, not just at the JSONL emit site. Even if a
# future regression caused two emits per attempt or skipped an emit, the
# invocation-count check would still catch a missing or extra adapter
# call.
#
...
PLAN="$FIXTURES/plans/plan-fail-four-times.md"
...
if [ "$ok" -ne 1 ]; then
  printf 'SUMMARY: p04-con5-no-fourth-record.sh pass=%d fail=1\n' "$pass"
  exit 1
fi

# --- Stage tmp_root + counter file (value=4 -> 4 failures pending; cap stops at 3) ---
TMP_ROOT="$(mktemp -d 2>/dev/null)"
if [ -z "$TMP_ROOT" ]; then
  TMP_ROOT="/tmp/p04-con5-fallback-$$"
...
DISPATCH_OUT_TMP="/tmp/p04-con5-out.txt"
DISPATCH_ERR_TMP="/tmp/p04-con5-err.txt"
...
USAGE_TMP="/tmp/p04-con5-usage.txt"
USAGE_LC_TMP="/tmp/p04-con5-usage-lc.txt"
...
CAP_TMP="/tmp/p04-con5-cap.txt"
CAP_LC_TMP="/tmp/p04-con5-cap-lc.txt"
...
ER_TMP="/tmp/p04-con5-er.txt"
ER_BAD_TMP="/tmp/p04-con5-er-bad.txt"
ER_BAD_LC_TMP="/tmp/p04-con5-er-bad-lc.txt"
rm -f "$ER_TMP" "$ER_BAD_TMP" "$ER_BAD_LC_TMP" 2>/dev/null
...
if [ "$ER_BAD_LC" = "0" ]; then
  ...
  fail=$((fail + 1))
  printf 'FAIL: %s records have non-verifier_fail escalation_reason\n' "$ER_BAD_LC"
fi
...
# --- Assertion 7: invocations file has exactly 3 lines (CON-5 hard-cap) ---
INV_LC_TMP="/tmp/p04-con5-inv-lc.txt"
rm -f "$INV_LC_TMP" 2>/dev/null
wc -l < "$INVOCATIONS_FILE" > "$INV_LC_TMP" 2>/dev/null
INV_LC="$(tr -d '[:space:]' < "$INV_LC_TMP")"
if [ "$INV_LC" = "3" ]; then
  pass=$((pass + 1))
  printf 'PASS: stub adapter invoked exactly 3 times (no fourth invocation)\n'
else
  fail=$((fail + 1))
  printf 'FAIL: stub adapter invoked %s times, expected 3 (CON-5 hard-cap violated)\n' "$INV_LC"
fi

# --- Cleanup ---
rm -f "$USAGE_TMP" "$USAGE_LC_TMP" "$CAP_TMP" "$CAP_LC_TMP" "$CAP_LINE_TMP" \
  "$USAGE_LINE3_TMP" "$ER_TMP" "$ER_BAD_TMP" "$ER_BAD_LC_TMP" "$INV_LC_TMP" \
...
printf 'SUMMARY: p04-con5-no-fourth-record.sh pass=%d fail=%d\n' "$pass" "$fail"
if [ "$fail" -eq 0 ]; then
  exit 0
fi
exit 1
