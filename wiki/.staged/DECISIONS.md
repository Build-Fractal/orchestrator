...
### Order of remaining milestones after [M015](milestones/M015/index.md) { #dr-code-004 }
...
- **Scope**: sequencing
- **Choice**: Revised sequence: M015 → M009 → [M011](milestones/M011/index.md) → [M012](milestones/M012/index.md) → [M013](milestones/M013/index.md) → [M014](milestones/M014/index.md) → M010. M010 (Cloud Dispatch / Managed Agents) moved from directly-after-M009 to the tail of the roadmap.
- **Revisable**: Yes — pull M010 forward if Managed Agents reaches GA before M014 completes.

Anthropic Managed Agents is not yet broadly available, so M010 value is externally gated. Meanwhile M011 (spec ingest), M012 (wiki), M013 (GitHub native integration), and M014 (comment automation) unlock the team's end-to-end spec→review→work loop, which is blocking current usability. M010's only technical prerequisite ([M008](milestones/M008/index.md) P03 dispatch adapter) is already satisfied, so order can be revisited without rework.

---

### Insert [M016](milestones/M016/index.md) + M017 into milestone sequence { #dr-code-005 }
...
### Slot [M018](milestones/M018/index.md) (Context Compression Layer) between M014 and M009 as a sketch { #dr-code-008 }
...
### Log mechanical criteria for [M020](milestones/M020/index.md) (Knowledge Layer Maturation) promote-or-dissolve decision at M012/P02 close { #dr-code-011 }
...
### Insert [M021](milestones/M021/index.md) (Autonomous Hardening v2) before M019 and position M019 Tier 1 emitter only after M021 closes { #dr-code-012 }
...
### Promote M023 (Design Layer) and [M024](milestones/M024/index.md) (Universal Intake & Routing) as committed milestones { #dr-code-016 }
...
### Capture two specify-flow friction points discovered during the M020 + [M026](milestones/M026/index.md) dogfood { #dr-code-020 }
...
- **Choice**: Yes — M020 holds exclusive schema authority over knowledge/spec/** and knowledge/**/MEM*.md frontmatter per FR-9. Introduce status: frontmatter field as a closed enum (candidate, graduated, archived); introduce decision_history: as an append-only YAML list of records carrying rationale, timestamp, operator, cluster_id; introduce archived_into: as a single canonical entry-ID back-reference. Pre-M020 entries lacking status: are treated as graduated on first read; the field is written on next touch (FR-10 incremental migration, no retroactive bulk pass). Vocabulary documented in [knowledge/conventions/MEM031.md](knowledge/conventions/MEM031.md). Consuming milestones (M024 universal intake, M019 Tier 2+3 observability) MAY READ these fields but MUST NOT introduce new fields without a follow-up M020 D-row.
...
### [D-RN-1](decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }") — npm package name `@build-fractal/orchestrator` { #dr-code-029 }
...
- **Scope**: rename
- **Choice**: npm publish target is `@build-fractal/orchestrator` (scoped). The unscoped `orchestrator` is taken on npm; collision check ran at M035/P00 (recorded in P00 SUMMARY) and confirmed `@build-fractal/orchestrator` available. Determines repo basename, binary name, and CLI command-cohort prefix downstream ([D-RN-2](decisions.md#d-rn-2-github-repo-basename-build-fractalorchestrator-dr-code-030 "GitHub repo basename `Build-Fractal/orchestrator` { #dr-code-030 }") through [D-RN-4](decisions.md#d-rn-4-homebrew-tap-build-fractalorchestrator-single-formula-dr-code-032 "Homebrew tap `build-fractal/orchestrator` (single-formula) { #dr-code-032 }")).
...
### [D-RN-2](decisions.md#d-rn-2-github-repo-basename-build-fractalorchestrator-dr-code-030 "GitHub repo basename `Build-Fractal/orchestrator` { #dr-code-030 }") — GitHub repo basename `Build-Fractal/orchestrator` { #dr-code-030 }
...
### [D-RN-3](decisions.md#d-rn-3-command-cohort-prefix-orchestratorcmd-dr-code-031 "Command-cohort prefix `orchestrator:<cmd>` { #dr-code-031 }") — Command-cohort prefix `orchestrator:<cmd>` { #dr-code-031 }
...
### [D-RN-4](decisions.md#d-rn-4-homebrew-tap-build-fractalorchestrator-single-formula-dr-code-032 "Homebrew tap `build-fractal/orchestrator` (single-formula) { #dr-code-032 }") — Homebrew tap `build-fractal/orchestrator` (single-formula) { #dr-code-032 }
...
Recorded at P01.5 to give P03 a stable target name; consumes [D-RN-1](decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }")'s `build-fractal` org choice.

---

### [D-RN-5](decisions.md#d-rn-5-local-clone-path-sitesorchestrator-dr-code-033 "Local clone path `~/Sites/orchestrator` { #dr-code-033 }") — Local clone path `~/Sites/orchestrator` { #dr-code-033 }
...
Drives [D-RN-6](decisions.md#d-rn-6-migrate-claude-memory-dir-alongside-path-rename-dr-code-034 "Migrate Claude memory dir alongside path rename { #dr-code-034 }") (Claude memory dir is derived from this path).

---

### [D-RN-6](decisions.md#d-rn-6-migrate-claude-memory-dir-alongside-path-rename-dr-code-034 "Migrate Claude memory dir alongside path rename { #dr-code-034 }") — Migrate Claude memory dir alongside path rename { #dr-code-034 }
...
- **Revisable**: Yes — `mv` the dir back if [D-RN-5](decisions.md#d-rn-5-local-clone-path-sitesorchestrator-dr-code-033 "Local clone path `~/Sites/orchestrator` { #dr-code-033 }") is reverted.

Off-tree operator step paired with [D-RN-5](decisions.md#d-rn-5-local-clone-path-sitesorchestrator-dr-code-033 "Local clone path `~/Sites/orchestrator` { #dr-code-033 }"); recorded so the T08 runbook documents both halves of the path rename.

---

### [D-RN-7](decisions.md#d-rn-7-pre-rename-version-tag-v09x-final-spec-kit-name-dr-code-035 "Pre-rename version tag `v0.9.X-final-spec-kit-name` { #dr-code-035 }") — Pre-rename version tag `v0.9.X-final-spec-kit-name` { #dr-code-035 }
...
- **Decided at**: [M035](milestones/M035/index.md) P03 plan-phase (2026-05-09).
...
1. **CON-7 / [M027](milestones/M027/index.md) alignment.** `update_source` joins the canonical
