...
- `scripts/knowledge/rebuild-index.sh` exists. Interface:
  - `bash scripts/knowledge/rebuild-index.sh --root <project-root>` — sets `PROJECT_ROOT=<root>`, then walks `<root>/knowledge/*/` (excluding `archive/`), writes `<root>/KNOWLEDGE-INDEX.md` atomically, and (re)builds the [M007](../../../../../milestones/M007/index.md) SQLite graph at `<root>/knowledge.db` via `lib/graph-db.sh`.
