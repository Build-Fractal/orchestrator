...
2. **Open `wiki/mkdocs.yml`** and locate the `plugins:` block. Find the `include-markdown` entry. P01 already added `rewrite_relative_urls: true` underneath it; assert that the exact string `rewrite_relative_urls: true` appears (spelling: underscore separators, lowercase `true`). If not present, add it as a sub-key under the plugin's options block:
   ...
   The include-plugin's `rewrite_relative_urls` option causes relative links inside the *included* markdown body (e.g., a link in [`.orchestrator/DECISIONS.md`](../../../../decisions.md) that reads `[M011 summary](milestones/M011/M011-SUMMARY.md)`) to be rewritten so they resolve against the *including* stub's URL instead of the canonical source path. This is the single load-bearing setting for cross-link navigation; the rest of the plugin config is already correct from P01.
...
5. **Extend the scanner** to emit `knowledge/**/MEM*.md` records. Add (Bash 3.2 safe, no `mapfile`, no `declare -A`, no process substitution):
   ...
   - `<rel-path>` is relative to repo root, matching the existing `.orchestrator/**.md` convention (every existing record's rel-path is also repo-root relative, e.g., [`.orchestrator/KNOWLEDGE.md`](../../../../knowledge.md)).
...
6. **Confirm scanner output order** — run the scanner once and inspect:
   ...
   Also spot-check: the last `archive:M###` record (if any) appears strictly before the first `knowledge:patterns` record. The first `knowledge:patterns` record's rel-path is [`knowledge/patterns/MEM001.md`](../../../../knowledge/patterns/MEM001.md) (lexical MEM id order).
...
- `bash scripts/wiki/wiki-scan-sources.sh` — stdout includes lines with `knowledge:patterns|[knowledge/patterns/MEM001.md](../../../../knowledge/patterns/MEM001.md)|…` style records; line count equals `.orchestrator/**.md` count plus `knowledge/**/MEM*.md` count.
...
- Manual: inspect scanner stdout for no empty third-field records (no `knowledge:patterns|[knowledge/patterns/MEM001.md](../../../../knowledge/patterns/MEM001.md)|` with trailing empty title).
...
- [`knowledge/patterns/MEM001.md`](../../../../knowledge/patterns/MEM001.md) through [`knowledge/patterns/MEM011.md`](../../../../knowledge/patterns/MEM011.md), [`knowledge/conventions/MEM012.md`](../../../../knowledge/conventions/MEM012.md) through `MEM020.md`, [`knowledge/lessons/MEM021.md`](../../../../knowledge/lessons/MEM021.md) through `MEM025.md` — per-entry MEM files with YAML frontmatter + `# MEM###: <Title>` H1. Body contents are the consolidated knowledge entries; P02 never duplicates them (AD-3 SSOT).
- `.orchestrator/memory/constitution.md` — Principle VIII (Bash 3.2 compat) applies to the scanner extension.
- [`.orchestrator/milestones/M012/M012-CONTEXT.md`](../../../../milestones/M012/M012-CONTEXT.md) — AD-1 (cross-refs only), AD-3 (include plugin SSOT) apply.
