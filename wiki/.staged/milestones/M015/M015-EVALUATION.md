...
- **Single-direction migration**: Hard cutover with no rollback feature flag (per [M007](../../milestones/M007/index.md) no-graceful-degradation rule). Lowers complexity at the design level (no dual code paths) but raises the bar for end-to-end validation in P04.
...
- **Pre-req validation already satisfied**: [M003](../../milestones/M003/index.md) P07/P08 ran end-to-end via `orchestrator-auto` in Claude Code native mode, confirming the standalone runtime works. Removes a major risk that would otherwise warrant Tier C.
