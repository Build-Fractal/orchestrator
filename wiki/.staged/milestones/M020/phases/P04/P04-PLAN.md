...
JSONL emission is intentionally NOT in P04's scope. Status.sh and compute-staleness.sh `--review-queue` are read-only by FR-8 / CON-1. [M019](../../../../milestones/M019/index.md) Tier 2+3 (downstream) consumes the existing P03 `knowledge_graduate` / `knowledge_archive` records for review-queue throughput metrics — no new event types are added in P04.
