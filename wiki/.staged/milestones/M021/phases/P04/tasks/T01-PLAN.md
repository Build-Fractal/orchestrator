...
- **Allow — at least 4 entries**: real [M011](../../../../../milestones/M011/index.md) tool-calls that shape-classifier passes through unchanged (no prompt in current system). Examples: `bash scripts/verify/run-suite.sh m011 P05`, `cat tmp/fixture.md`, `ls scripts/util/`, `bash a.sh && bash b.sh` (2-stage chain, under the threshold).
...
For each of the 20 screenshots cited in [`.orchestrator/milestones/M021/M021-CONTEXT.md`](../../../../../milestones/M021/M021-CONTEXT.md) AD-5 and the feature spec problem statement, transcribe the exact Bash tool-call string that Claude Code was about to dispatch. Where a call contains embedded newlines (heredocs, multi-line strings), encode them as literal `\n` inside the INPUT line.
...
- [`.orchestrator/milestones/M021/M021-CONTEXT.md`](../../../../../milestones/M021/M021-CONTEXT.md) — AD-2 (ten-pattern matrix), AD-3 (three-wrapper catalog), AD-5 (replay corpus is the authoritative regression gate), AD-9 (new AP-### entries for each Class B pattern). The 20-screenshot provenance is named here.
