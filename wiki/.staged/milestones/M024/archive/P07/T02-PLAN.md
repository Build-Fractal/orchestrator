...
3. Real-disk probe — checks `commands/design.md` exists AND its content contains a `Pass.<N>` marker (mirrors P03/T03's [M014](../../../../milestones/M014/index.md) probe pattern: `grep -E '^Pass\.[0-9]+' commands/design.md`). On both checks pass → `m023_shipped=true reason=disk-probe`. On either fail → `m023_shipped=false reason=disk-probe-failed`.
