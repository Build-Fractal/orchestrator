...
P03 closes the headline UX of M024: paragraph-shaped inputs run through a deep classifier, the operator sees an approve/cancel/revise gate, and approval routes to either `orchestrator:specify` (Tier B/C) or `orchestrator:dispatch` (Tier A) via two single-purpose route scripts. The M024→[M014](../../../../milestones/M014/index.md) handshake direction (per AD-4 direction `b`) is wired live against shipped M014/extended; an invoke-time probe degrades cleanly per #DQ-2 option `b` if M014 ever regresses on a future checkout.
