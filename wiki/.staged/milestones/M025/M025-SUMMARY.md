...
- **Knowledge-layer regression entry** — [`knowledge/lessons/MEM026.md`](../../knowledge/lessons/MEM026.md) cross-references commit `d33b8a7` and documents why M013/P04's gate suite missed the regression (no pre-existing-settings path exercised). [`knowledge/patterns/MEM027.md`](../../knowledge/patterns/MEM027.md) codifies the merge-not-overwrite pattern with the inline-tag + jq-optional + cascade-cleanup shape.
...
See `patterns_established` in frontmatter. The two new MEM entries (MEM026 lesson + MEM027 pattern) will compound into future user-scope-config-write phases: M010 Codex/Cursor parity (if those runtimes gain user-scope paths), [M019](../../milestones/M019/index.md) Tier 2+3 observability (if tier-2 JSONL ever writes under `$HOME`), any future runtime-agnostic hook registration work.
...
Phase-suite green (`pass=10 fail=0`). Four feat commits + one scaffolding commit. Forward roadmap advances to [M014](../../milestones/M014/index.md) (spec management extended).
