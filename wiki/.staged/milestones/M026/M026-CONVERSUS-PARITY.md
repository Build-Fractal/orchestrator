...
| Token / cost accounting (`engine/cost.py`) | `estimate_cost_usd` with per-model pricing, range estimate | `diff` empty — byte-identical | verified-identical | [M019](../../milestones/M019/index.md) Tier 2+3 plans that read cost estimates work on either edition |
