...
# Dispatch Context -- T04-phase-suite-and-demo (Phase P00, Milestone M027)
## Manifest
...
| Task Plan | 985-1087 | ~1800 | required |
| State Context | 1089-1095 | ~100 | required |
| First-Turn Completeness | 1097-1146 | ~900 | required |
| **Total** | | **~14200** | |
...
- `bash scripts/verify/m027-rollup-schema.sh` → exit 0, PASS line on stdout
- `bash scripts/verify/check-must-haves.sh .orchestrator/milestones/M027/phases/P00` → exit 0, all P00 must-haves green
- `bash scripts/diagnostics/metrics-rollup.sh --granularity milestone --milestone M019` → exit 0, one paired row on stdout
- `git diff --quiet` after the full suite run → exit 0 (FR-12 read-only invariant validated end-to-end)
