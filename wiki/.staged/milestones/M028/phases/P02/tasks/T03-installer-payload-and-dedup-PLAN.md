...
- The reject_lookup data lives inside `scripts/hooks/pre-bash-shape-guard.sh` as the `reject_lookup()` function (M021/P05 shape). There is no separate reject_lookup data file at the [M021](../../../../../milestones/M021/index.md) baseline; T03 does NOT extract it into a separate file. (P03 may add new reject_lookup entries via classifier extension; T03 ships the M021 baseline reject_lookup as part of the hook script itself.)
...
2. Insert the hooks-payload stage after the `--register` block and before the `--hook-config` capture. Verbatim shape:
    ...
    #   before-commit.sh          -- [M025](../../../../../milestones/M025/index.md) lifecycle script
