...
- Bash 3.2 compatibility (CON-7 carry-forward from `efficiency-footer.sh`): no associative arrays, no case-folding parameter expansion, no process substitution, no herestrings. Mirror the [M027](../../../../../milestones/M027/index.md) helpers' style.
...
The schema_version: "1.0" lock is the load-bearing public-contract decision. [M035](../../../../../milestones/M035/index.md) packaging consumes the schema as a post-install verification surface; post-launch `external-tool-adapters` consume it for GitHub Projects / Trello / Notion / Linear adapters. Any drift between the schema doc constant ("1.0") and the renderer constant (`_M029_SCHEMA_VERSION`) is a contract violation; the verifiers cross-check both literally.
