...
- `scripts/diagnostics/render-position.sh` exists, is executable, sources the AD-1 invocation-context resolver and emits a tree using the documented glyph set (`✓ ▶ ◇ ✗ ▽`) with milestone progress bar + per-row cost column, suppresses the cost column silently on pre-[M019](../../../../milestones/M019/index.md) milestones (FR-6/CON-3), and never invokes `gh` / GitHub APIs (FR-11/CON-4/SC-13).
