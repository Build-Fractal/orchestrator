...
- **Commit shape**: use `git commit -F <message-file>` rather than inline-HEREDOC `git commit -m "$(cat <<'EOF' ... EOF)"` per the M021/[M028](../../../../../milestones/M028/index.md) PreToolUse Bash shape-guard semantics documented in `CLAUDE.md` `## Commit Message Authoring`.
