...
1. **`commands/evaluate.md` drift fix (FR-14)** — remove the two pre-[M024](../../../../../milestones/M024/index.md) phrasings that contradict the post-M024 routing table:
...
4. **Confirm + amend `templates/orchestrator-config-default.yml`**.
   ...
   - If `quick_knowledge_token_budget` is not already present in the file (P01 should have added it), append a stanza below the `auto_proceed` line:
     ...
     # knowledge injection (enforced by [M018](../../../../../milestones/M018/index.md) tier-2 snip). Default 800.
