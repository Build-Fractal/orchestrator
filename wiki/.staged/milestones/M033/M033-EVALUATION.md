...
2. **Paired-launch coordination with [M032](../../milestones/M032/index.md) (CON-1) requires roadmap-level decomposition with explicit per-phase coordination.** M033/P05 invokes M032's `--with-wiki` gate; M033's grilling-shell writes inline into M032's `wiki/glossary.md`; SC-9 has explicit two-mode (P01..P04 stub / P05 real) acceptance contract per MIT-001. The integration testing cadence across the three load-bearing seams cannot fit a single phase.
...
- **Knowledge-layer boundary spanning [M020](../../milestones/M020/index.md) / [M031](../../milestones/M031/index.md) / M032 / M033**: FR-7 writes into M020's existing knowledge-graph kinds (no new kinds); FR-7 + FR-8 produce MEMs that M031's `build-context.sh --profile=quick` consumes; FR-18 writes inline into M032's `wiki/glossary.md`. Four-way boundary documented explicitly in the spec.
- **CC-only launch posture (NG-equivalent)**: M033's interactive flows (FR-3 / FR-9 / FR-10 / FR-13) inherit [M030](../../milestones/M030/index.md)'s adaptive model routing for surgical-character tasks; M030's closure is a hard upstream dependency.
...
- **[M015](../../milestones/M015/index.md) (standalone cutover, closed)** — provides `orchestrator:migrate --from <kind>` that FR-11 pre-fills the flag for.
- **[M013](../../milestones/M013/index.md) (GitHub native integration, closed)** — provides `orchestrator:github-init` that FR-16 pass-throughs to.
- **[M014](../../milestones/M014/index.md) / spec 035 dual-write Recent Changes contract** — FR-21 inherits the dual-write helper.
- **[M027](../../milestones/M027/index.md) / [M019](../../milestones/M019/index.md) observability infrastructure** — FR-22's structured records consume the existing JSONL append + cost-rollup consumers.
