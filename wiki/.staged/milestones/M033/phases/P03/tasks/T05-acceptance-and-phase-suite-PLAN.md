...
2. **Author `tests/m033-acceptance/p03-ingest-codebase.sh`** (≥100 lines, executable, `chmod +x`). The script (SC-3):
   ...
   2e. **FR-8 / MIT-005 rich-context assertion.** For the `m033-stack-fixture-ts-saas` staging dir (which carries [`.orchestrator/DECISIONS.md`](../../../../../decisions.md) with `DR-DEMO-001` and `DR-DEMO-002`):
...
6. **Author `tools/verify/m033-p03-scope-guard.sh`** (≥50 lines, executable). Bidirectional scope-guard (P02/T05 pattern):
   ...
   - **`wiki/` boundary (M033-tagged narrowing per P01 D-T05-02):** scan `wiki/` for filenames matching `M033*` or `m033*` — assert none exist (P03 should not write to wiki/; that's [M032](../../../../../milestones/M032/index.md) territory). Note: the orchestrator's pre-existing `wiki/` content is unrelated and is NOT scanned.
...
- A summary file at [`.orchestrator/milestones/M033/phases/P03/tasks/T05-acceptance-and-phase-suite-SUMMARY.md`](../../../../../milestones/M033/phases/P03/tasks/T05-acceptance-and-phase-suite-SUMMARY.md) documents the deliverables.
