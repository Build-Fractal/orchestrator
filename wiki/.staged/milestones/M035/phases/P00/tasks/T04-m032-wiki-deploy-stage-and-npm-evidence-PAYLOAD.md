...
# Dispatch Context -- T04-m032-wiki-deploy-stage-and-npm-evidence (Phase P00, Milestone M035)
## Manifest
...
| Task Plan | 985-1147 | ~3200 | required |
| State Context | 1149-1155 | ~100 | required |
| First-Turn Completeness | 1157-1192 | ~600 | required |
| **Total** | | **~15400** | |
...
- `scripts/lifecycle/wiki-init.sh emit_pages_workflow()` HEREDOC includes a `bash scripts/diagnostics/wiki-stubs-fresh.sh` step before `mkdocs build` in newly-emitted `pages.yml` workflows; the existing-workflow preservation branch (CON-3 at line 484) is unchanged.
  - Check: `bash tools/verify/m035-p00-wiki-stubs-fresh.sh`
- `scripts/lifecycle/wiki-init.sh` `--deploy` step 2 stages `$REPO_ROOT/scripts/wiki/wiki-deploy.sh` into `$PROJECT_DIR/scripts/wiki/wiki-deploy.sh` when the project copy is missing, before invoking it (closes [M032](../../../../../milestones/M032/index.md) SC-5 fixture-completeness gap).
  - Check: `bash tools/verify/m035-p00-wiki-deploy-stage.sh`
- `packaging/bundle/D-RN-1-evidence.txt` exists and records the `npm view @build-fractal/orchestrator` and `npm view orchestrator` outcomes captured at P00 plan-time, alongside the resolution `[D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }"): @build-fractal/orchestrator (unscoped name taken)`.
...
- `npm` is on PATH on the plan-author's machine for the collision-check command. If not present, the plan-author records the gap in the evidence file and notes the manual command to re-run when npm is available — the resolution is the same: per RENAME-PLAN [D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }"), `@build-fractal/orchestrator` is the default fallback when unscoped `orchestrator` is taken (which it is, per user's confirmation in discuss-step).
...
2. **[D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }") npm-name collision-check evidence** — record the `npm view @build-fractal/orchestrator` and `npm view orchestrator` outcomes in `packaging/bundle/D-RN-1-evidence.txt` so P01.5 can rename without re-running the check and so the evidence is auditable.
...
2. **Author `packaging/bundle/D-RN-1-evidence.txt`**. Run the collision check and record the result:
   ...
   ```
   [D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }") npm-name collision-check evidence
   ...
   Resolution: [D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }") = @build-fractal/orchestrator (RENAME-PLAN.md default fallback when unscoped is taken; user-confirmed at discuss-step 2026-05-07).

   Downstream binding:
   - [D-RN-2](../../../../../decisions.md#d-rn-2-github-repo-basename-build-fractalorchestrator-dr-code-030 "GitHub repo basename `Build-Fractal/orchestrator` { #dr-code-030 }") GitHub repo basename: Build-Fractal/orchestrator (RENAME-PLAN match-npm-scope rule).
   - [D-RN-4](../../../../../decisions.md#d-rn-4-homebrew-tap-build-fractalorchestrator-single-formula-dr-code-032 "Homebrew tap `build-fractal/orchestrator` (single-formula) { #dr-code-032 }") Homebrew tap: build-fractal/orchestrator (RENAME-PLAN single-formula tap default).
   ...
   - Tag v0.9.X-final-spec-kit-name immediately before P01.5 rename commits land ([D-RN-7](../../../../../decisions.md#d-rn-7-pre-rename-version-tag-v09x-final-spec-kit-name-dr-code-035 "Pre-rename version tag `v0.9.X-final-spec-kit-name` { #dr-code-035 }")).
   - Migrate ~/.claude/projects/-Users-brettkellgren-Sites-spec-kit-orchestrator/ → -Sites-orchestrator/ ([D-RN-6](../../../../../decisions.md#d-rn-6-migrate-claude-memory-dir-alongside-path-rename-dr-code-034 "Migrate Claude memory dir alongside path rename { #dr-code-034 }")).
   ```
...
4. **Author `tools/verify/m035-p00-npm-collision-evidence.sh`**. The verifier:
   ...
   - Asserts the file contains the substrings [`D-RN-1`](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }"), `@build-fractal/orchestrator`, `Resolution:`, and `Captured at:`.
...
- `references/RENAME-PLAN.md` — pre-existing decision-runbook authoritative for [D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }")..[D-RN-7](../../../../../decisions.md#d-rn-7-pre-rename-version-tag-v09x-final-spec-kit-name-dr-code-035 "Pre-rename version tag `v0.9.X-final-spec-kit-name` { #dr-code-035 }"). T04's evidence file references it.
...
- The phase-suite aggregator MUST embed `m035-p00-` in its filename — the unprefixed `p00-phase-suite.sh` shape silently clobbered prior milestones' aggregators ([M030](../../../../../milestones/M030/index.md) lost to [M031](../../../../../milestones/M031/index.md), M031 lost to [M036](../../../../../milestones/M036/index.md)) and is forbidden by AD-19 path discipline.
...
- **Why three deliverables in one task**: the wiki-deploy stage fix is ~10 lines of installer-edit; the [D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }") evidence file is a ~25-line text artifact; the phase-suite aggregator is a ~30-line shell script. Each is too small for its own task plan, and the phase-suite aggregator's correctness depends on T01/T02/T03 verifiers existing — it's the natural last-task wrap-up. Standard intensity (2–4 tasks per phase) sized cleanly to four tasks; combining these three into one keeps the boundary clean.
- **[D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }") follow-up at P01.5**: if `npm view @build-fractal/orchestrator` returns "name reserved by another scope owner" rather than 404, P01.5 plan-phase re-decides [D-RN-1](../../../../../decisions.md#d-rn-1-npm-package-name-build-fractalorchestrator-dr-code-029 "npm package name `@build-fractal/orchestrator` { #dr-code-029 }") with operator input. The evidence file's `(unverified — re-run before P01.5)` annotation handles this case explicitly.
...
- **Milestone**: M035
- **Phase**: P00
- **Task**: T04-m032-wiki-deploy-stage-and-npm-evidence
- **Tier**: C
