...
- **Extraction ownership** — orchestrator owns Tier 0 (manifest + binary preservation), Tier 1 (deterministic shell adapters: PDF/DOCX/XLSX/Markdown), Tier 2 (LLM-driven structured Markdown via [M030](../../milestones/M030/index.md) routing + conversus fidelity gate). NG-3, NG-4, NG-5 inverted/narrowed accordingly.
...
This work lands as **M036** — its own milestone, sequential after the queued [M031](../../milestones/M031/index.md)–[M035](../../milestones/M035/index.md). Three positions were considered (spec Open Question #Q-1):
...
- (b) Re-open [M020](../../milestones/M020/index.md) as M020.1 — rejected; closed milestones are not idiomatic to re-open in this project.
...
- **M036b** (P08–P09, 2 phases) — post-launch, demand-driven. P08 wiki projection (depends on [M032](../../milestones/M032/index.md) closure). P09 operator-facing scale UX (REVIEW queue, change-over-time queries, supersede chain at scale). Ships when validator-pilot feedback surfaces concrete operator pain.
...
- **[M011](../../milestones/M011/index.md) (spec-management) / M020 (knowledge-layer maturation)** — knowledge tree, `rebuild-index.sh`, `traverse-graph.sh`. Re-used; additively extended. **Hard dependency** on the existing spec-chunk machinery being functional (it is — both closed).
- **[M005](../../milestones/M005/index.md) / [M018](../../milestones/M018/index.md) (dispatch + compression-tier)** — context recipe, scope-filter, dispatch context-builder, tokenizer. Re-used for the `reference:` payload section and budget governor. **Hard dependency** on the recipe schema being declarative (Principle X) — already true.
- **[M012](../../milestones/M012/index.md) / M032 (wiki tooling)** — MkDocs integration. M032 is **in queue** (pre-launch). **Soft dependency**: M036 wiki-projection phase consumes M032's `--with-wiki` plumbing; if M036 lands before M032 wraps, this phase blocks. Roadmap calls this out as a sequencing dependency.
- **[M013](../../milestones/M013/index.md) / [M014](../../milestones/M014/index.md) (GitHub native + spec management gate work)** — initial reading: **NO hard dependency** (per spec Open Question #Q-6). M013/M014's gates fire on spec-shape conformity and Section Contract fidelity, neither of which apply to reference content (which has its own taxonomy + provenance contract). Soft integration points (dual-write helper, [M027](../../milestones/M027/index.md) metrics surface) are useful but not load-bearing.
