...
# Dispatch Context -- T01-schema-and-indexer (Phase P05, Milestone M036)
## Manifest
...
| Task Plan | 1041-1232 | ~3000 | required |
| State Context | 1234-1240 | ~100 | required |
| First-Turn Completeness | 1242-1279 | ~700 | required |
| **Total** | | **~17400** | |
...
- `scripts/knowledge/lib/graph-db.sh` exists (M020) and contains `CHECK(edge_type IN ('relates_to', 'supersedes'))` at line 73.
...
- **Verifier path discipline** — all three new verifiers MUST live under `tools/verify/` (project-owned per-phase verifiers, slug-bearing). All three MUST use the milestone-prefix `m036-p05-*` to avoid collision with the existing M030 `p05-*.sh` verifier set.
