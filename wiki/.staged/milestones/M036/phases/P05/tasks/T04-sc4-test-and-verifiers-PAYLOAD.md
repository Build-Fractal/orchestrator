...
# Dispatch Context -- T04-sc4-test-and-verifiers (Phase P05, Milestone M036)
## Manifest
...
| Task Plan | 1041-1266 | ~3000 | required |
| State Context | 1268-1274 | ~100 | required |
| First-Turn Completeness | 1276-1313 | ~700 | required |
| **Total** | | **~17400** | |
...
The phase-suite aggregator follows the same shape as `tools/verify/m036-p00-phase-suite.sh` (P00 deliverable, recovered after the M031→M036 collision incident). It runs each sub-gate, counts pass/fail, and emits one `SUMMARY:` line.
...
- **Path-collision discipline** — every artifact T04 creates uses the milestone-prefixed `m036-p05-*.sh` slug. The pre-existing M030 `p05-*.sh` set is unaffected. Re-verify at task-execution time with `ls -la tools/verify/m036-p05-*.sh tests/test-reference-graph-edges.sh` — none should exist before T04 starts (they were declared `create` in the phase plan).
...
- **Milestone**: M036
- **Phase**: P05
- **Task**: T04-sc4-test-and-verifiers
- **Tier**: C
