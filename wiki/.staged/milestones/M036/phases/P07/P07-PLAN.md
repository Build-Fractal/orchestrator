...
- `tests/test-reference-backwards-compat-golden.sh` (create) — SC-7 acceptance harness ([M030](../../../../milestones/M030/index.md) SC-11 shape); runs build-context.sh against the no-scope fixture; asserts byte-equality against the captured pre-feature baseline. Emits `BATTERY: pass=N fail=N skip=N` last line.
...
- Existing tokenizer access — `scripts/dispatch/build-context.sh` already includes a `(_section_chars + 3) / 4` token-estimation helper at the manifest-builder layer ([M018](../../../../milestones/M018/index.md) compression-tier). `reference_apply_budget` re-uses the same per-chunk token estimate (counts characters, divides by 4) for consistency. No new tokenizer dependency.
