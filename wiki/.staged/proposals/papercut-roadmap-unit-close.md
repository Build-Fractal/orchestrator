...
After running `orchestrator:roadmap` end-to-end for milestone [M032](../milestones/M032/index.md) (Tier C, with finalized context draft, producing `M032-ROADMAP.md` with 4 phases + cross-cutting concerns + dependency graph + execution order + validation per `templates/roadmap.md`), `.orchestrator/execution-log.jsonl` contains zero records reflecting the run. The same holds for the immediately-following [M033](../milestones/M033/index.md) roadmap run.

`grep -E 'orchestrator:roadmap|"command":"roadmap"|roadmap.*unit_close' .orchestrator/execution-log.jsonl` returns nothing — neither in the M030/[M031](../milestones/M031/index.md) historical run-up nor in today's M032/M033 sessions.
...
- `orchestrator:dispatch` — emits `unit_close` per dispatched task (Tier 1 observability, [M019](../milestones/M019/index.md))
- `orchestrator:verify` — emits `unit_close` per verification run ([M027](../milestones/M027/index.md) cost rollup consumer)
...
- M027 efficiency-footer + [M030](../milestones/M030/index.md) adaptive-model selection both consume per-command cost/quality signals and have no signal for roadmap stages — they cannot tune intensity, model choice, or budget for a stage they cannot observe.
- [M029](../milestones/M029/index.md) (roadmap visibility & CLI UX) is *the* downstream consumer of roadmap state (`orchestrator:where` tree renderer reads the roadmap file). When M029 starts asking "when was this roadmap last regenerated?", `git log` answers that question but the per-run metadata (intensity used, time spent, model invocation count, validation-section pass/fail breakdown) is not recoverable.
...
3. Confirm the roadmap file is authored: `ls [.orchestrator/milestones/M032/M032-ROADMAP.md](../milestones/M032/M032-ROADMAP.md)`.
